...
1. **Report-packing function (now the top priority — the button/ADC read-path investigation is mostly closed).** Session 8 fully mapped the GPIO button-IRQ chain via the NVIC vector table (`analysis/bruce-io-paths.md`'s new "GPIO button-IRQ dispatch chain" section): confirmed only the 8 *combined-bank* GPIO IRQs (GPIO1–5) have real handlers — ADC1/ADC2, ADC_ETC, and the individual per-pin GPIO IRQs are all wired to the same default/unused spin handler, so ADC is definitively **not** interrupt-driven (polling or DMA, still unlocated) and buttons definitively **are** combined-bank-IRQ-driven. Traced the 8 ISR trampolines through a previously-undefined shared ITCM dispatcher (`FUN_60049290`, flash 0x60049290) that walks a 32-bit fired-pin bitmap and invokes a per-pin registered `(fn_ptr, arg)` callback from a flat 160-slot table. Confirmed `FUN_60071624` (the session-6 lead) as the NVIC-registration call via its callee `FUN_60060040` (writes the real NVIC ISER register at 0xE000E100, IRQ range 80–89 exactly matches the combined-GPIO-bank IRQs). Found the callback table is part of one big board-singleton C++ object at fixed RAM 0x200064c0, which per `bruce-itcm.md`'s RAM layout is `.bss` — so the actual per-pin callback function pointers aren't present anywhere in the static image and must be set by a not-yet-located runtime constructor. **That constructor call is now the precise next target**; once found, it should lead directly to the button-state-update code and from there to the still-unidentified report-packing function (combines calibrated stick/trigger + button state into the final 11-byte report ID 3 buffer — `hid_input_target.cc` only transmits an already-built report).
