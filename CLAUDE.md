...
1. **Report-packing function (still the top priority; the button-IRQ callback-registration thread is now closed, with a twist).** Session 8 fully mapped the GPIO button-IRQ chain via the NVIC vector table: only the 8 *combined-bank* GPIO IRQs (GPIO1–5) have real handlers → a shared ITCM dispatcher (`FUN_60049290`, flash 0x60049290) that walks a 32-bit fired-pin bitmap and invokes a registered `(fn_ptr, arg)` callback from a flat 160-slot table inside the board-singleton C++ object at fixed RAM `0x200064c0` (`.bss`, so no callback values exist statically). **Session 9 found the constructor that populates those table slots — `timer__60073bf0` @ 0x60073bf0 (the input/HID-task object constructor) — but it turns out to write a generic literal no-op (`FUN_600ce232` = raw `bx lr`, confirmed by disassembly, reused 34× elsewhere in the image as the codebase's default "unset handler" idiom) with arg 0, not a real handler.** Also corrected a session-8 mis-read along the way: the table stores one *shared context object* (`board+0x38b8`), not a per-pin pointer — all 17 registered button pins dispatch through the exact same callback regardless of which pin fired. Whether a real handler ever overrides that no-op is unresolved (checked ~500 of ~5,000 decompiled functions and a direct RAM-literal search of the whole image — no override found either way, inconclusive either direction). Two hypotheses, not yet distinguished: (1) a not-yet-found `SetCallback`-style setter overrides it once the input task starts, or (2) the no-op is intentional and these interrupts exist only to wake the CPU from sleep, with real button/ADC sampling done by a still-unlocated periodic polling task. **Precise next target: find `timer__60073bf0`'s sibling that actually starts/runs the input/HID task** (a FreeRTOS `xTaskCreate`-shaped call taking the same `0x200064c0`-rooted object) — full detail and the raw-disassembly evidence in `analysis/bruce-io-paths.md`'s "GPIO button-IRQ callback registration (session 9)" section.
