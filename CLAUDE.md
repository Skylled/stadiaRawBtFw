...
5. **Now the top active priority: continue the BTA/BTE stack sweep** (`analysis/bruce-bta-stack.md`) — still ~700+ of 1,076 functions in the block remain, though the highest-value "named via leaked string table" ones are now mostly exhausted (unnamed/generic helpers likely remain, lower value per function — the GATT-server engine cluster at `0x6006axxx`/`0x6008xxxx` decompiled during session 12's report-packing search is a good continuation point, already has real momentum). L2CAP is located (`0x600b5xxx`–`0x600b9fxx`) but not deeply read. BTA_GATTS server API surface now substantially covered (session 12).
6. ~~`bcm.c` cheap win~~ — **done (session 9):** all 52 functions decompiled and read. Turned out to be a misattribution, not a Broadcom BT driver — it's BoringSSL's BIGNUM + generic EC (`EC_GROUP`/`EC_POINT`/`EC_KEY`) library code (`analysis/bruce-crypto.md`'s new session-9 section has the full function map and evidence).
...
8. `analysis/bruce-decompile-status.md` was last regenerated session 8 (35.4%/47.7% figures) — sessions 9-12 added ~100 more decompiled functions since, so it's mildly stale again. Low urgency; regenerate when convenient.
9. Longer term: apply the same workflow to the `gotham` Wi-Fi firmware (the cloud-streaming stack).
10. **Session 14: found the codebase-wide log/diagnostic `Buffer` framework** (`analysis/bruce-log-buffer.md`) — a small message-buffer class + rate-limited ring-buffer emission engine called directly from 64 of 139 attributed source files (the widest call-graph reach of anything found so far, wider than BTA/BTE), and very likely the source of the `__FILE__` leaks this whole project's attribution technique depends on. Open thread from that doc: the registered-sink array `FUN_6010162e` walks (candidate transport for the adjacent `StreamLogBuffer`/`DumpDeviceInformation` debug-command strings at `0x6010277c`–`0x601027ec`) wasn't traced — natural next step. Same session also found two smaller extensions of existing subsystems (more BoringSSL runtime support at decompile-status run #3, and the GKI buffer-pool allocator underlying BTA/BTE at run #5) — both noted in that doc rather than written up separately.
