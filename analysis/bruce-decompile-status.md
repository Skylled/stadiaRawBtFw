...
|---:|---|---|---:|---:|---:|---:|---:|---|
| 1 | `0x600921b8` | `0x600c9cc4` | 228108 | 204449 | 1076 | 2 | 89.6% | `FUN_600ba1c4` (3898B @ `600ba1c4`) |
...
**#1 is the headline finding: `0x600921b8`–`0x600c9cc4`, 204,449 bytes of dense (89.6%) code across 1,076 functions, with only 2 decompiled so far.** This is by a wide margin the single largest block of totally-unmapped, clearly-real code in the image — bigger by actual code bytes than every other run combined except #2. It already contains two functions flagged as leads in `bruce-ghidra.md`'s "Next-session targets": `FUN_600ba1c4` (byte-buffer parser, guessed HCI/GATT/SDP framing) and `FUN_600c8fd8` (big-number arithmetic, guessed P-256 field ops for BLE Secure Connections ECDH) — both sit inside this exact range. **Recommended next move: treat this range as a single module-identification target** (find its `__FILE__` strings / xrefs / a natural entry point) rather than working function-by-function — at 1,076 functions it's too big for retail decompile-and-read.
