...
**Snapshot: 2026-08-18 (full regeneration, session 152 / Wave 122).** This is a **full regeneration**, not a hand-patch — every table below comes from a fresh address-based join against `bruce_functions.csv`, `bruce_srcmap.csv`, and a directory listing of `analysis/decomp/` taken at the conclusion of session 152 (3637 decomp files, 3637 in-census). Decompiled in-census functions grew to **3637** (530,870 bytes, 74.28% of code), with **139 of 139** attributed source files fully decompiled.

## 1. Top-line stats

| Metric | Functions | % of 5,569 | Bytes | % of 714,723 |
|---|---:|---:|---:|---:|
| **Total functions (census)** | 5,569 | 100% | 714,723 | 100% |
| **Attributed** (leaked `__FILE__` → 139 src files) | 448 | 8.04% | 96,312 | 13.48% |
| **Decompiled** (`analysis/decomp/*.c`, matched to census) | 3,637 | 65.31% | 530,870 | 74.28% |
| **Attributed ∪ Decompiled** (understood in *some* way) | 3,637 | 65.31% | 530,870 | **74.28%** |
| **Totally unknown** (no attribution, no decompile) | 1,932 | 34.69% | 183,853 | **25.72%** |
...
| | Functions | % of 5,569 | Bytes | % of 714,723 |
|---|---:|---:|---:|---:|
| Totally unknown (§1, includes BTA-identified-but-undecompiled) | 1,932 | 34.69% | 183,853 | 25.72% |
| — of which: BTA-identified, module known, just not decompiled | 0 | 0.00% | 0 | 0.00% |
| **— truly unidentified (no attribution, no decompile, no module ID)** | **1,932** | **34.69%** | **183,853** | **25.72%** |
...
| 2 | `0x600ecb50` | `0x6013d4e4` | 330132 | 97911 | 1049 | 671 | 29.7% | `FUN_601054dc` (2546B @ `601054dc`) |
...
Of **5,569 total functions** (the current census):

| | Functions | Bytes |
|---|---:|---:|
| Attributed to a source file | 448 | 96,312 |
| Decompiled (in census) | 3,637 | 530,870 |
...
| **Understood in some way (union)** | **3,637 (65.3%)** | **530,870 (74.3%)** |
| **Completely unknown — no attribution, no decompile, no module ID** | 1,932 (34.7%), or **1,932 (34.7%) excluding BTA-identified** | 183,853 (25.7%), or **183,853 (25.7%) excluding BTA-identified** |
