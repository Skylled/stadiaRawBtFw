...
**Snapshot: 2026-08-18 (full regeneration, session 91 / Wave 61).** This is a **full regeneration**, not a hand-patch — every table below comes from a fresh address-based join against `bruce_functions.csv`, `bruce_srcmap.csv`, and a directory listing of `analysis/decomp/` taken at the conclusion of session 91 (2734 decomp files, 2734 in-census). Decompiled in-census functions grew to **2734** (439,524 bytes, 61.48% of code), with **139 of 139** attributed source files fully decompiled.

## 1. Top-line stats

| Metric | Functions | % of 5,537 | Bytes | % of 714,923 |
|---|---:|---:|---:|---:|
| **Total functions (census)** | 5,537 | 100% | 714,923 | 100% |
| **Attributed** (leaked `__FILE__` → 139 src files) | 448 | 8.09% | 97,450 | 13.63% |
| **Decompiled** (`analysis/decomp/*.c`, matched to census) | 2,734 | 49.38% | 439,524 | 61.48% |
| **Attributed ∪ Decompiled** (understood in *some* way) | 2,734 | 49.38% | 439,524 | **61.48%** |
| **Totally unknown** (no attribution, no decompile) | 2,803 | 50.62% | 275,399 | **38.52%** |
...
| | Functions | % of 5,537 | Bytes | % of 714,923 |
|---|---:|---:|---:|---:|
| Totally unknown (§1, includes BTA-identified-but-undecompiled) | 2,803 | 50.62% | 275,399 | 38.52% |
| — of which: BTA-identified, module known, just not decompiled | 0 | 0.00% | 0 | 0.00% |
| **— truly unidentified (no attribution, no decompile, no module ID)** | **2,803** | **50.62%** | **275,399** | **38.52%** |
...
| 3† | `0x600df286` | `0x600ea868` | 46562 | 44492 | 330 | 180 | 95.6% | `FUN_600e398a` (6270B @ `600e398a`) |
...
Of **5,537 total functions** (the current census):

| | Functions | Bytes |
|---|---:|---:|
| Attributed to a source file | 448 | 97,450 |
| Decompiled (in census) | 2,734 | 439,524 |
...
| **Understood in some way (union)** | **2,734 (49.4%)** | **439,524 (61.5%)** |
| **Completely unknown — no attribution, no decompile, no module ID** | 2,803 (50.6%), or **2,803 (50.6%) excluding BTA-identified** | 275,399 (38.5%), or **275,399 (38.5%) excluding BTA-identified** |
