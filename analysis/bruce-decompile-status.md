...
**Snapshot: 2026-08-18 (full regeneration, session 72 / Wave 42).** This is a **full regeneration**, not a hand-patch — every table below comes from a fresh address-based join against `bruce_functions.csv`, `bruce_srcmap.csv`, and a directory listing of `analysis/decomp/` taken at the conclusion of session 72 (2354 decomp files, 2354 in-census). Decompiled in-census functions grew to **2354** (410,410 bytes, 57.35% of code), with **139 of 139** attributed source files fully decompiled.

## 1. Top-line stats

| Metric | Functions | % of 5,539 | Bytes | % of 715,659 |
|---|---:|---:|---:|---:|
| **Total functions (census)** | 5,539 | 100% | 715,659 | 100% |
| **Attributed** (leaked `__FILE__` → 139 src files) | 448 | 8.09% | 97,450 | 13.62% |
| **Decompiled** (`analysis/decomp/*.c`, matched to census) | 2,354 | 42.50% | 410,410 | 57.35% |
| **Attributed ∪ Decompiled** (understood in *some* way) | 2,354 | 42.50% | 410,410 | **57.35%** |
| **Totally unknown** (no attribution, no decompile) | 3,185 | 57.50% | 305,249 | **42.65%** |
...
| | Functions | % of 5,539 | Bytes | % of 715,659 |
|---|---:|---:|---:|---:|
| Totally unknown (§1, includes BTA-identified-but-undecompiled) | 3,185 | 57.50% | 305,249 | 42.65% |
| — of which: BTA-identified, module known, just not decompiled | 0 | 0.00% | 0 | 0.00% |
| **— truly unidentified (no attribution, no decompile, no module ID)** | **3,185** | **57.50%** | **305,249** | **42.65%** |
...
| 6† | `0x600d8a12` | `0x600df24c` | 26682 | 24368 | 432 | 186 | 91.3% | `FUN_600dcf8c` (984B @ `600dcf8c`) |
...
| Decompiled (in census) | 2,354 | 410,410 |
...
| **Understood in some way (union)** | **2,354 (42.5%)** | **410,410 (57.3%)** |
| **Completely unknown — no attribution, no decompile, no module ID** | 3,185 (57.5%), or **3,185 (57.5%) excluding BTA-identified** | 305,249 (42.7%), or **305,249 (42.7%) excluding BTA-identified** |
