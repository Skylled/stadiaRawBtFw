...
**Snapshot: 2026-08-18 (full regeneration, session 132 / Wave 102).** This is a **full regeneration**, not a hand-patch — every table below comes from a fresh address-based join against `bruce_functions.csv`, `bruce_srcmap.csv`, and a directory listing of `analysis/decomp/` taken at the conclusion of session 132 (3389 decomp files, 3389 in-census). Decompiled in-census functions grew to **3389** (502,256 bytes, 70.28% of code), with **139 of 139** attributed source files fully decompiled.
...
| **Decompiled** (`analysis/decomp/*.c`, matched to census) | 3,389 | 60.89% | 502,256 | 70.28% |
| **Attributed ∪ Decompiled** (understood in *some* way) | 3,389 | 60.89% | 502,256 | **70.28%** |
| **Totally unknown** (no attribution, no decompile) | 2,177 | 39.11% | 212,345 | **29.72%** |
...
|---|---:|---:|---:|---:|
| Totally unknown (§1, includes BTA-identified-but-undecompiled) | 2,177 | 39.11% | 212,345 | 29.72% |
| — of which: BTA-identified, module known, just not decompiled | 0 | 0.00% | 0 | 0.00% |
| **— truly unidentified (no attribution, no decompile, no module ID)** | **2,177** | **39.11%** | **212,345** | **29.72%** |
...
| 2 | `0x600ecb50` | `0x6013d4e4` | 330132 | 97789 | 1046 | 423 | 29.6% | `FUN_601054dc` (2546B @ `601054dc`) |
...
| Decompiled (in census) | 3,389 | 502,256 |
...
| **Understood in some way (union)** | **3,389 (60.9%)** | **502,256 (70.3%)** |
| **Completely unknown — no attribution, no decompile, no module ID** | 2,177 (39.1%), or **2,177 (39.1%) excluding BTA-identified** | 212,345 (29.7%), or **212,345 (29.7%) excluding BTA-identified** |
