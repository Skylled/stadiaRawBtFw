...
**Snapshot: 2026-08-18 (full regeneration, session 82 / Wave 52).** This is a **full regeneration**, not a hand-patch — every table below comes from a fresh address-based join against `bruce_functions.csv`, `bruce_srcmap.csv`, and a directory listing of `analysis/decomp/` taken at the conclusion of session 82 (2554 decomp files, 2554 in-census). Decompiled in-census functions grew to **2554** (423,000 bytes, 59.13% of code), with **139 of 139** attributed source files fully decompiled.

## 1. Top-line stats

| Metric | Functions | % of 5,538 | Bytes | % of 715,319 |
|---|---:|---:|---:|---:|
| **Total functions (census)** | 5,538 | 100% | 715,319 | 100% |
| **Attributed** (leaked `__FILE__` → 139 src files) | 448 | 8.09% | 97,450 | 13.62% |
| **Decompiled** (`analysis/decomp/*.c`, matched to census) | 2,554 | 46.12% | 423,000 | 59.13% |
| **Attributed ∪ Decompiled** (understood in *some* way) | 2,554 | 46.12% | 423,000 | **59.13%** |
| **Totally unknown** (no attribution, no decompile) | 2,984 | 53.88% | 292,319 | **40.87%** |
...
| | Functions | % of 5,538 | Bytes | % of 715,319 |
|---|---:|---:|---:|---:|
| Totally unknown (§1, includes BTA-identified-but-undecompiled) | 2,984 | 53.88% | 292,319 | 40.87% |
| — of which: BTA-identified, module known, just not decompiled | 0 | 0.00% | 0 | 0.00% |
| **— truly unidentified (no attribution, no decompile, no module ID)** | **2,984** | **53.88%** | **292,319** | **40.87%** |
...
| 6† | `0x600d8a12` | `0x600df24c` | 26682 | 24028 | 431 | 386 | 90.1% | `FUN_600dcf8c` (984B @ `600dcf8c`) |
...
| Decompiled (in census) | 2,554 | 423,000 |
...
| **Understood in some way (union)** | **2,554 (46.1%)** | **423,000 (59.1%)** |
| **Completely unknown — no attribution, no decompile, no module ID** | 2,984 (53.9%), or **2,984 (53.9%) excluding BTA-identified** | 292,319 (40.9%), or **292,319 (40.9%) excluding BTA-identified** |
