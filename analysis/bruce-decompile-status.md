...
**Snapshot: 2026-08-18 (full regeneration, session 44 / Wave 14).** This is a **full regeneration**, not a hand-patch — every table below comes from a fresh address-based join against `bruce_functions.csv`, `bruce_srcmap.csv`, and a directory listing of `analysis/decomp/` taken at the conclusion of session 44 (1862 decomp files, 1862 in-census). Decompiled in-census functions grew to **1862** (378,527 bytes, 52.86% of code), with **139 of 139** attributed source files fully decompiled.

## 1. Top-line stats

| Metric | Functions | % of 5,542 | Bytes | % of 716,074 |
|---|---:|---:|---:|---:|
| **Total functions (census)** | 5,542 | 100% | 716,074 | 100% |
| **Attributed** (leaked `__FILE__` → 139 src files) | 448 | 8.08% | 97,450 | 13.61% |
| **Decompiled** (`analysis/decomp/*.c`, matched to census) | 1,862 | 33.60% | 378,527 | 52.86% |
| **Attributed ∪ Decompiled** (understood in *some* way) | 1,862 | 33.60% | 378,527 | **52.86%** |
| **Totally unknown** (no attribution, no decompile) | 3,680 | 66.40% | 337,547 | **47.14%** |
...
|---|---:|---:|---:|
| BTA/BTE block total | 1,122 | 214,239 | 100% |
| — decompiled (also counted in §1's "Decompiled" row) | 1,093 | 207,571 | 96.9% |
| — **identified but not yet decompiled** | 29 | 6,668 | 3.1% |

Netting this out against §1's "Totally unknown" row gives the true unidentified figure:

| | Functions | % of 5,542 | Bytes | % of 716,074 |
|---|---:|---:|---:|---:|
| Totally unknown (§1, includes BTA-identified-but-undecompiled) | 3,680 | 66.40% | 337,547 | 47.14% |
| — of which: BTA-identified, module known, just not decompiled | 29 | 0.52% | 6,668 | 0.93% |
| **— truly unidentified (no attribution, no decompile, no module ID)** | **3,651** | **65.88%** | **330,879** | **46.21%** |
...
|---:|---|---|---:|---:|---:|---:|---:|---|
| 1★ | `0x600921b8` | `0x600c9cc4` | 228108 | 214239 | 1122 | 1093 | 96.9% | `FUN_600ba1c4` (3898B @ `600ba1c4`) |
...
Of **5,542 total functions** (the current census):

| | Functions | Bytes |
|---|---:|---:|
| Attributed to a source file | 448 | 97,450 |
| Decompiled (in census) | 1,862 | 378,527 |
| — of which both attributed AND decompiled | 448 | 97,450 |
| — of which in the identified-but-not-formally-attributed BTA/BTE stack (§1a) | 1,093 | 207,571 |
| **Understood in some way (union)** | **1,862 (33.6%)** | **378,527 (52.9%)** |
| **Completely unknown — no attribution, no decompile, no module ID** | 3,680 (66.4%), or **3,651 (65.9%) excluding BTA-identified** | 337,547 (47.1%), or **330,879 (46.2%) excluding BTA-identified** |
