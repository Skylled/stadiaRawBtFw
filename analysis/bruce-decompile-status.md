...
**Snapshot: 2026-08-18 (full regeneration, session 139 / Wave 109).** This is a **full regeneration**, not a hand-patch — every table below comes from a fresh address-based join against `bruce_functions.csv`, `bruce_srcmap.csv`, and a directory listing of `analysis/decomp/` taken at the conclusion of session 139 (3474 decomp files, 3474 in-census). Decompiled in-census functions grew to **3474** (513,108 bytes, 71.80% of code), with **139 of 139** attributed source files fully decompiled.
...
| **Decompiled** (`analysis/decomp/*.c`, matched to census) | 3,474 | 62.41% | 513,108 | 71.80% |
| **Attributed ∪ Decompiled** (understood in *some* way) | 3,474 | 62.41% | 513,108 | **71.80%** |
| **Totally unknown** (no attribution, no decompile) | 2,092 | 37.59% | 201,493 | **28.20%** |
...
|---|---:|---:|---:|---:|
| Totally unknown (§1, includes BTA-identified-but-undecompiled) | 2,092 | 37.59% | 201,493 | 28.20% |
| — of which: BTA-identified, module known, just not decompiled | 0 | 0.00% | 0 | 0.00% |
| **— truly unidentified (no attribution, no decompile, no module ID)** | **2,092** | **37.59%** | **201,493** | **28.20%** |
...
| 2 | `0x600ecb50` | `0x6013d4e4` | 330132 | 97789 | 1046 | 508 | 29.6% | `FUN_601054dc` (2546B @ `601054dc`) |
...
| Decompiled (in census) | 3,474 | 513,108 |
...
| **Understood in some way (union)** | **3,474 (62.4%)** | **513,108 (71.8%)** |
| **Completely unknown — no attribution, no decompile, no module ID** | 2,092 (37.6%), or **2,092 (37.6%) excluding BTA-identified** | 201,493 (28.2%), or **201,493 (28.2%) excluding BTA-identified** |
