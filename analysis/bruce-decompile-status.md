...
**Snapshot: 2026-08-18 (full regeneration, session 156 / Wave 126).** This is a **full regeneration**, not a hand-patch — every table below comes from a fresh address-based join against `bruce_functions.csv`, `bruce_srcmap.csv`, and a directory listing of `analysis/decomp/` taken at the conclusion of session 156 (3682 decomp files, 3682 in-census). Decompiled in-census functions grew to **3682** (537,416 bytes, 75.18% of code), with **139 of 139** attributed source files fully decompiled.

## 1. Top-line stats

| Metric | Functions | % of 5,570 | Bytes | % of 714,861 |
|---|---:|---:|---:|---:|
| **Total functions (census)** | 5,570 | 100% | 714,861 | 100% |
| **Attributed** (leaked `__FILE__` → 139 src files) | 448 | 8.04% | 96,312 | 13.47% |
| **Decompiled** (`analysis/decomp/*.c`, matched to census) | 3,682 | 66.10% | 537,416 | 75.18% |
| **Attributed ∪ Decompiled** (understood in *some* way) | 3,682 | 66.10% | 537,416 | **75.18%** |
| **Totally unknown** (no attribution, no decompile) | 1,888 | 33.90% | 177,445 | **24.82%** |
...
| | Functions | % of 5,570 | Bytes | % of 714,861 |
|---|---:|---:|---:|---:|
| Totally unknown (§1, includes BTA-identified-but-undecompiled) | 1,888 | 33.90% | 177,445 | 24.82% |
| — of which: BTA-identified, module known, just not decompiled | 0 | 0.00% | 0 | 0.00% |
| **— truly unidentified (no attribution, no decompile, no module ID)** | **1,888** | **33.90%** | **177,445** | **24.82%** |
...
| 2 | `0x600ecb50` | `0x6013d4e4` | 330132 | 98049 | 1050 | 716 | 29.7% | `FUN_601054dc` (2546B @ `601054dc`) |
...
Of **5,570 total functions** (the current census):

| | Functions | Bytes |
|---|---:|---:|
| Attributed to a source file | 448 | 96,312 |
| Decompiled (in census) | 3,682 | 537,416 |
...
| **Understood in some way (union)** | **3,682 (66.1%)** | **537,416 (75.2%)** |
| **Completely unknown — no attribution, no decompile, no module ID** | 1,888 (33.9%), or **1,888 (33.9%) excluding BTA-identified** | 177,445 (24.8%), or **177,445 (24.8%) excluding BTA-identified** |
