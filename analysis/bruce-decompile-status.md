...
**Snapshot: 2026-08-18 (full regeneration, session 138 / Wave 108).** This is a **full regeneration**, not a hand-patch — every table below comes from a fresh address-based join against `bruce_functions.csv`, `bruce_srcmap.csv`, and a directory listing of `analysis/decomp/` taken at the conclusion of session 138 (3465 decomp files, 3465 in-census). Decompiled in-census functions grew to **3465** (511,940 bytes, 71.64% of code), with **139 of 139** attributed source files fully decompiled.
...
| **Decompiled** (`analysis/decomp/*.c`, matched to census) | 3,465 | 62.25% | 511,940 | 71.64% |
| **Attributed ∪ Decompiled** (understood in *some* way) | 3,465 | 62.25% | 511,940 | **71.64%** |
| **Totally unknown** (no attribution, no decompile) | 2,101 | 37.75% | 202,661 | **28.36%** |
...
|---|---:|---:|---:|---:|
| Totally unknown (§1, includes BTA-identified-but-undecompiled) | 2,101 | 37.75% | 202,661 | 28.36% |
| — of which: BTA-identified, module known, just not decompiled | 0 | 0.00% | 0 | 0.00% |
| **— truly unidentified (no attribution, no decompile, no module ID)** | **2,101** | **37.75%** | **202,661** | **28.36%** |
...
| 2 | `0x600ecb50` | `0x6013d4e4` | 330132 | 97789 | 1046 | 499 | 29.6% | `FUN_601054dc` (2546B @ `601054dc`) |
...
| Decompiled (in census) | 3,465 | 511,940 |
...
| **Understood in some way (union)** | **3,465 (62.3%)** | **511,940 (71.6%)** |
| **Completely unknown — no attribution, no decompile, no module ID** | 2,101 (37.7%), or **2,101 (37.7%) excluding BTA-identified** | 202,661 (28.4%), or **202,661 (28.4%) excluding BTA-identified** |
