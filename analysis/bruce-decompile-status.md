...
**Snapshot: 2026-08-18 (full regeneration, session 164 / Wave 134).** This is a **full regeneration**, not a hand-patch — every table below comes from a fresh address-based join against `bruce_functions.csv`, `bruce_srcmap.csv`, and a directory listing of `analysis/decomp/` taken at the conclusion of session 164 (3832 decomp files, 3832 in-census). Decompiled in-census functions grew to **3832** (549,520 bytes, 76.86% of code), with **139 of 139** attributed source files fully decompiled.

## 1. Top-line stats

| Metric | Functions | % of 5,572 | Bytes | % of 714,987 |
|---|---:|---:|---:|---:|
| **Total functions (census)** | 5,572 | 100% | 714,987 | 100% |
| **Attributed** (leaked `__FILE__` → 139 src files) | 448 | 8.04% | 96,312 | 13.47% |
| **Decompiled** (`analysis/decomp/*.c`, matched to census) | 3,832 | 68.77% | 549,520 | 76.86% |
| **Attributed ∪ Decompiled** (understood in *some* way) | 3,832 | 68.77% | 549,520 | **76.86%** |
| **Totally unknown** (no attribution, no decompile) | 1,740 | 31.23% | 165,467 | **23.14%** |
...
| | Functions | % of 5,572 | Bytes | % of 714,987 |
|---|---:|---:|---:|---:|
| Totally unknown (§1, includes BTA-identified-but-undecompiled) | 1,740 | 31.23% | 165,467 | 23.14% |
| — of which: BTA-identified, module known, just not decompiled | 0 | 0.00% | 0 | 0.00% |
| **— truly unidentified (no attribution, no decompile, no module ID)** | **1,740** | **31.23%** | **165,467** | **23.14%** |
...
| 2 | `0x600ecb50` | `0x6013d4e4` | 330132 | 98175 | 1052 | 866 | 29.7% | `FUN_601054dc` (2546B @ `601054dc`) |
...
Of **5,572 total functions** (the current census):

| | Functions | Bytes |
|---|---:|---:|
| Attributed to a source file | 448 | 96,312 |
| Decompiled (in census) | 3,832 | 549,520 |
...
| **Understood in some way (union)** | **3,832 (68.8%)** | **549,520 (76.9%)** |
| **Completely unknown — no attribution, no decompile, no module ID** | 1,740 (31.2%), or **1,740 (31.2%) excluding BTA-identified** | 165,467 (23.1%), or **165,467 (23.1%) excluding BTA-identified** |
