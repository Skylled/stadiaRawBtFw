...
| Address | Bytes | Identification | Confidence |
|---|---:|---|---|
| `0x6008c0d8` | ~~628~~ ⚠️ 600 | `BN_mod_exp`-shaped: sliding-window modexp, stack power-table (up to 32 entries), square-and-multiply main loop reading exponent bits via a `BN_is_bit_set`-shaped callee — ⚠️ **byte count corrected, QA session 93**: a fresh census join found this address's committed decomp file (`bcm__6008c0d8.c`, from the original session-9 sweep) now disagrees with the current Ghidra census (600 bytes, not 628); disassembly confirms a clean, real function prologue (`push.w {r4-r8,sb,sl,fp,lr}; sub sp,#0x54`) starting exactly at offset `0x25c`(=600) from this address, i.e. 600 is the accurate current boundary and the committed `.c` file is stale — it drifted during some later session's unrelated `FixSpuriousSplits.java`/`FixTailCalls.java` housekeeping, undocumented in that session's own commit message. `GHIDRA-TODO`: re-decompile `bcm__6008c0d8` at its true 600-byte size. | high |
| `0x6008c334` | 706 | constant-time windowed modexp (`BN_mod_exp_mont_consttime`-shaped): heap-allocated flat power table + fixed-width table-gather via the raw-word-copy helper, i.e. the constant-time-access variant — ✅ **independently corroborated, QA session 93**: this function's own callee `crypto_bn_set_bit` (`0x600e73ac`, `bruce-misc-functions.md`) was found this session to actually be a constant-time "select the *N*-th of `2^k` precomputed-table entries" helper — precisely the "fixed-width table-gather" this entry already predicted, from a totally independent investigation 84 sessions later. | high |
...
| `0x6008d6f8` | ~~292~~ ⚠️ 52 | ~~Jacobian→affine coordinate recovery: field inversion via a Fermat's-little-theorem exponentiation chain, calling the group's `field_mul`/`field_sqr` method-vtable slots directly, branching on the `a=-3` flag~~ ⚠️ **corrected QA session 101** — a fourth instance of the undocumented-boundary-shrink pattern (after `bcm__6008c0d8`/session 93 and `bcm__6008db08`/session 100): this session's commit shrank the census entry 292→52 without mentioning it. Disassembly of the true 52 bytes shows only a **method-check guard** (`0x600eb428`, error-log-and-return-0 on failure) that on success **tail-calls** (`b.w`) this same session's new `crypto_ec_point_is_on_curve_jacobian` (`0x600eb280`, 240 bytes — `52+240=292`, exactly the stale claimed size) with a `+4` pointer offset. There is no Fermat-inversion/coordinate-recovery logic in this function at all; the old 292-byte decompile was tail-call bleed-through of `0x600eb280`'s body. Real identity: the public-API-style guard wrapper for `EC_POINT_is_on_curve`, not coordinate recovery. `GHIDRA-TODO`: re-decompile `bcm__6008d6f8` at 52 bytes (same fix already proven on the two prior instances via `FixTailCalls.java`). | medium |
...
| `0x6008db08` | ~~300~~ ⚠️ 68 | ~~`EC_POINT_cmp`-shaped: method-match guard + point-equality compare, returns `-1` on mismatch~~ ⚠️ **corrected QA session 100** — a fresh census join found this address's committed decomp file (still 300 bytes, referencing this same-session-added `0x600eadd0`) disagrees with the current census (68 bytes); the 300→68 shrink landed in session 100's own commit, undocumented in its message — the same "boundary corrected in Ghidra, `.c` file not re-run" pattern as sessions 93/95's `bcm__6008c0d8` regressions. Disassembly of the real 68 bytes confirms it is only the **method-match guard** half (two calls to `FUN_600eb428`, an established method-check, erroring via the confirmed diagnostic-log call on mismatch) — on success it **tail-calls** `FUN_600eb370` (`b.w`, not `bl`) with pointer offsets `+4`, which is where the actual "point-equality compare" logic now lives; the old 300-byte decompile was tail-call bleed-through of `FUN_600eb370`'s body, not genuinely part of this function. `GHIDRA-TODO`: re-decompile `bcm__6008db08` at 68 bytes; `FUN_600eb370` is the real comparison core and is not yet independently examined. This also means session 90's `crypto_ed25519_key_compare` row (`bruce-misc-functions.md`, `0x600e084e`) calling this function "a constant-time comparison helper" is imprecise — the actual comparison is one tail-call further, in `FUN_600eb370`. | high |
...
⚠️ **REFUTED — corrected QA session 88, found while verifying an unrelated adjacent wave (`crypto_asn1_error_dispatch_6`/`_198` at `0x600e06e6`/`0x600e06f6`), reopening this thread.** `FUN_600e06ee`'s committed decomp is a stale (session 4/5, predates the whole tail-call-bleed artifact class this pipeline later learned to catch — see sessions 49–56) rendering: disassembly of its real 8 bytes shows `mov r2,r1; movs r1,#0x74; b.w 0x600e06d0` — a trampoline, not a direct call to `evp__6008506c`. `0x600e06d0` (undefined in the census — no Function boundary, hence the bleed) is the *real*, shared caller of `evp__6008506c`, and it has **at least three genuine sibling trampolines calling it**, not one: `0x600e06e6` (code `6`=RSA, session 87), `0x600e06ee` (code `0x74`=DSA, this stale file), and `0x600e06f6` (code `0x198`=EC, session 88) — all disassembly-confirmed by the identical `mov r2,r1; mov[.w] r1,#code; b.w 0x600e06d0` shape. Critically, **`0x600e06f6`'s trampoline (the EC/`0x198` path) has a confirmed real caller, `FUN_60085528`** (180 bytes, not yet independently traced) — directly contradicting "no callers found." The "dead-ends... for every algorithm" conclusion does not hold as stated: the EC path is now confirmed one hop closer to live than previously believed. **Whether `FUN_60085528` itself is reachable from a live bruce entry point is the new open question** — not yet resolved, but this is no longer a confirmed dead end at the `evp__6008506c` level. **GHIDRA-TODO: define a function boundary at `0x600e06d0` (real body `push{r3,r4,r5,lr}; ...; bl evp__6008506c; ...; pop{r3,r4,r5,pc}`, ending `0x600e06e4`), mark the three tail-call edges at `0x600e06d0`/`0x600e06ee`(via `0x600e06f2`)/`0x600e06f6`(via `0x600e06fc`) as explicit `CALL_RETURN` overrides (the proven `FixTailCalls.java` technique), and re-decompile `FUN_600e06ee` — its true size is 8 bytes, not the currently-committed 30.** Next step for whoever picks this thread back up: trace `FUN_60085528`'s own callers.

**Prior conclusion** (superseded by the above, kept for history): this was believed to be the generic BoringSSL EVP/ASN.1 algorithm-registry machinery pulled in wholesale because gotham's TLS 1.3/DTLS/X.509 stack needs it (`firmware-map.md`: bruce shares 168 of gotham's 173 source files) — present in bruce's link but with no static-analysis-confirmed live caller, consistent with it being linked-but-dormant rather than an active bruce feature. That conclusion no longer holds for the EC (`0x198`) path specifically; Ed25519/X25519 (`0x3b5`/`0x3b4`) are unaffected by this correction and remain unconfirmed live, since neither NID appears among the three trampolines found so far.
