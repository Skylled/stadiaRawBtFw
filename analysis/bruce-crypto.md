...
- **`tasn_dec__6008f370`** (310B) — `asn1_check_tlen`-shaped: parses/verifies a BER/DER tag-length header, caching the parsed length/tag/class fields into a caller-owned "context" struct (`param_11`) so a re-entrant caller can skip re-parsing. Called from every other function in the file.
...
- **`tasn_dec__6008fa18`** (1410B, largest) — the real recursive `asn1_item_embed_d2i` core: switches on the `ASN1_ITEM`'s `itype` (0=PRIMITIVE, 1/5=SEQUENCE/NDEF_SEQUENCE, 2=CHOICE, 3=MSTRING, 4=EXTERN — the multi-string tag-bitmask lookup at case 5's `DAT_6008fcf8` table matches `ASN1_ITYPE_MSTRING`'s accepted-tag bitmap idiom exactly), walking an `ASN1_TEMPLATE` array for SEQUENCE/CHOICE members and recursing per field.
