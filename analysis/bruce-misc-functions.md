...
| `0x600ed656` |   28 | Crypto / String | **`crypto_cbs_is_valid_code_point`** — BoringSSL Unicode code point validator: validates `cp < 0x110000`, noncharacter exclusion (`cp & 0xFFFE != 0xFFFE` and `cp` not in `0xFDD0..0xFDEF`), and UTF-16 surrogate exclusion (`cp` not in `0xD800..0xDFFF`). | 4 callers / 0 callees |
