...
⚠️ **File-collision data gap, flagged QA session 66 — GHIDRA-TODO.** Two of this wave's 20 claimed addresses have no dedicated committed `.c` file: `0x600d9378` and `0x600d937c` are both genuine census entries (4 bytes each, confirmed) whose decompiles were named by *target* only (`thunk_EXT_FUN_00007d64.c`, `thunk_EXT_FUN_00007dac.c`) instead of the collision-safe `thunk_<name>__<owncaddr>.c` convention `Decompile.java` adopted in session 57 specifically to prevent this. As a result each file reflects only the *second* of its two colliding source addresses (`thunk_EXT_FUN_00007d64.c`'s header shows `600d938e`, not `600d9378`; `thunk_EXT_FUN_00007dac.c`'s header shows `600d938a`, not `600d937c`) — the first address's decompile was silently never separately committed. Lower severity than session 56's `private_heap_free` incident (no pre-existing, unrelated content was overwritten — both colliding addresses are brand-new this wave and are genuine tail-branch duplicates into the *same* real target, so no unique information is lost), but it does mean the wave's "20 functions" byte/count claim double-counts by file coverage: only 18 distinct `.c` files exist for the 20 claimed addresses. The byte-total math still reconciles (both missing addresses' census sizes are included in the join), so no total-byte correction is needed. **GHIDRA-TODO: re-run `Decompile.java` on `0x600d9378` and `0x600d937c` with disambiguated filenames** (`thunk_EXT_FUN_00007d64__600d9378.c`, `thunk_EXT_FUN_00007dac__600d937c.c`) so both addresses get their own committed file, and check why this wave's thunk decompiles didn't use the session-57 naming fix (a regression in the tooling path used, not a one-off).
