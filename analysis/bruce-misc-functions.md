...
| Address | Bytes | Subsystem | Functional Role & Evidence | Call graph |
|---|---:|---|---|---|
| `0x600f033a` |  168 | BTA / DM | **`bta_dm_acl_change`** — ACL connection change handler: finds device block via `0x6009feb8` or `0x600f1746`, updates mode count, copies 8-byte descriptors via `0x6013d3a0` (`memcpy`), updates link policy via `0x600a8494`, and conditionally calls `0x600bc7e8` (`l2cu_update_lcb_4_gatts`). | 3 callers / 5 callees |
| `0x600f03e2` |   36 | BTA / DM PM | **`bta_dm_pm_sniff_mode_event`** — Sniff mode event forwarder: calls `0x600b29b8` (`BTM_PM_OnSniffSubratingEvt`). | 2 callers / 1 callee |
| `0x600f0406` |   88 | BTA / DM PM | **`bta_dm_pm_set_sniff_policy`** — Sniff policy setter: queries link block via `0x60098ec8` (`l2cu_find_lcb_by_bd_addr`), sets latency at `+0x10c`, and invokes `0x600b3a80` (`BTM_SetQoS`). | 2 callers / 2 callees |
...
| `0x600f04e4` |  110 | BTA / DM PM | **`bta_dm_pm_disconnect_link`** — Link disconnect dispatcher: gets HCI handle via `0x600f048a`, checks device record via `0x6009ff18`, and calls `0x600b218c` (`BTM_SecDisconnect` / `btm_sec_disconnect` with reason `0x13`). | 2 callers / 3 callees |
...
| `0x600f0662` |   44 | BTA / DM BLE | **`bta_dm_ble_set_bg_conn_type`** — Background connection type setter: converts boolean `param_2` (0 or 9) and delegates to `0x600c0640` (`bta_dm_ble_set_bg_conn_type`). | 1 caller / 1 callee |
| `0x600f068e` |   78 | BTA / DM BLE | **`bta_dm_ble_observe`** — BLE observation starter: queries device record via `0x6009ff18`, sets flag `0x200` at `+0x2a`, calls `0x600c0698` (`bta_dm_ble_observe`). | 1 caller / 2 callees |
| `0x600f06dc` |   74 | BTA / DM BLE | **`bta_dm_ble_scan`** — BLE scan starter: queries device record via `0x6009ff18`, sets flag `0x200` at `+0x2a`, calls `0x600c073c` (`bta_dm_ble_scan`). | 1 caller / 2 callees |
