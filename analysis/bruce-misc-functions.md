...
| `0x600cdbbc` |  20 | CRT / Reent | ~~**`__getreent`** — Retrieves per-thread reentrancy structure errno pointer `*(reent + 0xec)`.~~ ⚠️ **misidentified, corrected QA session 165** — a real `__getreent()` returns the reentrancy-struct pointer itself; this function instead performs an *extra* dereference, returning `*(reent + 0xec)` — a field pulled **out of** the struct, not the struct pointer. Two independent, freshly-decompiled callers (session 164's `ctype_is_xdigit_or_blank`/`0x601020ec` and session 165's `strcasecmp_custom`/`0x6010244e`) both use the return value as a ctype-table base pointer with the exact newlib convention (`table[c+1]`, the `+1` offset that lets `EOF`=-1 index safely) — and `strcasecmp_custom`'s bit test is a precise, falsifiable match to real BSD/newlib ctype bits: `(table[c+1] & 3) == 1` selects `_U`(0x01) set / `_L`(0x02) clear — "uppercase, not lowercase" — before adding the literal `0x20` to fold to lowercase, exactly `strcasecmp`'s real case-folding step. Real identity is very likely the per-reentrancy-context ctype-table-pointer accessor (newlib's internal `__locale_ctype_ptr`-shaped helper), not `__getreent`. | 3 callers / 0 callees |
...
| `0x600ef476` |   56 | BTA / GATTC | ~~**`bta_gattc_register_notify_filter`** — Notification registration filter: registers notification filter callback at `*(param_1 + 0x14)` and sets registration flag bit 0 at `+0x18`.~~ ⚠️ **too narrow, corrected QA session 155** — its own body (`bool FUN_600ef476(int param_1, undefined4 param_2) { bVar1 = *(int*)(param_1+0x14)==0; if (bVar1) { *(int*)(param_1+0x14)=param_2; *(byte*)(param_1+0x18) |= 1; } return bVar1; }`) is a generic "claim the single pending-operation slot if empty" guard, not anything notification-specific — and this doc's own sessions 154–155 now show it called, with the identical 2-argument pattern, as the *entry gate* of at least 6 more, completely unrelated GATTC client action handlers (read-by-UUID, read-multiple, service-discovery-start, write-char-value, execute-write, and the generic `bta_gattc_q_cmd` action-table helper), each storing its *own* incoming event-data pointer into `+0x14` — not a notification filter callback. This also lines up with `0x600ef3cc`'s already-corrected (session 112) description of the *same* `+0x14` field as a "pending GATT command" slot. Real identity: a generic single-slot "claim/reserve the pending-command slot for this CLCB" guard used across the whole GATTC client action-table dispatch layer, not specific to notification-filter registration. Left unconfirmed as to a formal name rather than guessed. | 6 callers / 0 callees |
...
**⚠️ QA session 145 note:** Backbone clean — all 12 addresses/sizes match the census exactly, summing to the claimed 1,658 bytes, fully contiguous `0x600faec4`–`0x600fb53e`, zero gaps/overlaps. Independently re-derived `bruce-decompile-status.md`'s totals via a fresh header-parsed join (3,536 functions / 522,776 bytes, 0 mismatches, 0 duplicates, 0 non-census addresses) — matches the wave's claimed 73.15% exactly.

**Zero corrections — an exceptional match to the real Bluetooth SMP wire formats across nearly every function.** `0x600faec4`'s six unpacked Pairing Request fields (IO Capability, OOB flag, AuthReq, MaxEncKeySize, InitiatorKeyDistribution, ResponderKeyDistribution) match both the real SMP Pairing Request PDU's field order and this row's own claimed byte offsets exactly. `0x600fb0ac`/`0x600fb110`/`0x600fb1e6`/`0x600fb24a` all correctly store 16-byte values (Confirm, Random, DHKey Check, Commitment) — matching the real SMP spec's field sizes for each — with matching flag bits set at each. `0x600fb160`'s two 32-byte coordinate copies (64 bytes total) match the real ECC P-256 public key's X/Y coordinate format exactly. `0x600fb346`'s 2-byte EDIV + 8-byte Rand split matches the real SMP Master Identification PDU exactly (10 bytes total). `0x600fb43a`'s 1-byte address type + 6-byte reversed BD_ADDR matches the real SMP Identity Address Information PDU exactly, using this pipeline's long-established BD_ADDR-reversal convention.

**Strong cross-function/cross-session field-offset consistency**: `0x600fb30a` writes the peer LTK to `+0x1c6`, the exact same offset session 143's confirmed `btm_sec_link_key_notification` (`0x600fac38`) reads from to persist a link key — a genuine, spec-consistent LTK/Link-Key aliasing relationship, confirmed from both the writer's and reader's side across two sessions. `0x600fb3fe` writes the peer IRK to `+0x1b6`, the exact offset this same wave's own `0x600fb43a` reads from when processing the associated Identity Address — a sensible pairing (IRK and Identity Address are used together for BLE identity resolution), confirmed within one wave.

**One soft, unresolved observation (not flagged as an error)**: three of this wave's functions (`0x600fb346`/`0x600fb43a`/`0x600fb4d0`) pass small values (`1`/`2`/`4`) as the second argument to `0x6009ad00`, matching the same values passed to `0x600fa7de` in the same functions — but sessions 143/144's similarly-shaped functions (`0x600fac38`/`0x600facc4`/`0x600fad26`) passed larger, byte-count-shaped values (`0x10`/`0x20`/`0x40`) to the same `0x6009ad00` call for what appear to be the same key types. This wave's write-up avoids asserting a specific byte-size for these three save calls (unlike sessions 143/144, one of which was corrected last session for exactly this kind of claim), so there's nothing here to correct — but the inconsistency suggests `0x6009ad00`'s second parameter may be a type/slot code in some call sites and a byte count in others (or a single semantic this QA hasn't yet fully resolved). Worth a closer look if `0x6009ad00` itself is ever decompiled.
...
| `0x600fb9d2` |   52 | BTM / SMP | **`btm_sec_process_key_missing`** — Broadcom SMP handle link key missing condition: ~~resets crypto state (`0x600fc4d8`)~~ ⚠️ **wrong, corrected QA session 149** — `0x600fc4d8` is now attributed (session 149) as `smp_sc_calculate_dhkey`, computing the P-256 Diffie-Hellman shared secret via EC scalar multiplication — a real cryptographic computation, not a state reset. Real effect of this unconditional first call is "(re)compute the SC DHKey," not "reset crypto state"; the enclosing function's own "link key missing" framing is correspondingly less certain than described, since starting with an SC-specific DHKey (re)computation doesn't obviously fit a legacy/BR-EDR "link key missing" scenario — left as unconfirmed rather than reassigned., triggers passkey retry event 12 (`0x600fabc6`), and dispatches event `0x1e`. | 0 callers / 3 callees |
...
| Address | Bytes | Subsystem | Functional Role & Evidence | Call graph |
|---|---:|---|---|---|
| `0x600fba06` |  150 | BTM / SMP | **`btm_sec_verify_sc_commitment`** — Broadcom SMP Secure Connections verify pairing value: compares local calculated value against received value `*(param_1 + 0xfd)` (16 bytes via `memcmp`), adjusts max key size, handles master (`0x600c1a34(param_1, 0xd, 0)`) vs slave (`0x600c1a34(param_1, 0x18, 0)`), or triggers failure `0x17` with status `0x0b` (~~`SMP_PAIR_FAIL_CONFIRM_VALUE`~~ ⚠️ **wrong spec name, corrected QA session 147** — the real Bluetooth SMP Pairing Failed reason code `0x0B` is **DHKey Check Failed**, not Confirm Value Failed (which is `0x04`, correctly used by the already-established sibling `0x600fb53e`/`btm_sec_verify_confirm` two sessions ago). Since firmware must send the spec-exact reason byte to the peer for pairing to fail correctly, this is a real, checkable contradiction, not a stylistic quibble — it raises real doubt about whether this function's own overall identity is "verify commitment" (Cb/Ca, checked during Authentication Stage 1) versus "verify DHKey check" (Ea/Eb, checked after DHKey computation), since the failure code it actually sends matches the latter. Left as unconfirmed rather than reassigning outright — the offset `0xfd` doesn't independently disambiguate. ). | 0 callers / 2 callees |
...
| `0x600fbb14` |  172 | BTM / SMP | **`smp_sc_init_passkey_round`** — Broadcom SMP Secure Connections initialize passkey / numeric comparison authentication state: splits 32-bit passkey into 4-byte buffers, resets round index `*(param_1 + 0x199) = 0`, and ~~starts passkey round crypto (`0x600c1548`)~~ ⚠️ **wrong callee characterization, corrected QA session 147** — `0x600c1548` is already independently established (`bruce-bta-stack.md`) as `smp_set_state_phase3_enc`, a generic 3-caller SMP FSM state-transition helper (2 of its 3 callers are unrelated to SC passkey rounds) that requests one block of `btsnd_hcic_ble_rand` entropy and unconditionally sets the FSM's sub-state field (`+0x1f2`) to a fixed value (13, "Phase 3 link encryption start") — no AES-CMAC/f4/f5/f6 call anywhere in it, nothing round-specific. Real effect of this call is "request more randomness and advance the FSM to Phase 3," not "passkey round crypto." (The real LE Secure Connections Passkey Entry protocol genuinely does use 20 authentication rounds per spec — matching a 20-bit passkey — but nothing in either this function's or `0x600c1548`'s own body evidences that count; the "20-round" figure in this row's own name is asserted from general spec knowledge, not from decompiled evidence.). | 0 callers / 1 callee |
...
**⚠️ QA session 147 note:** Backbone: all 15 addresses/sizes match the census exactly and are fully contiguous `0x600fba06`–`0x600fbe82`, including the two Fix #79/#80 restorations — disassembled the whole `0x600fbbc0`–`0x600fbc50` stub cluster directly and confirmed all four "empty stub" functions (`0x600fbbc0`, `0x600fbbd6`, `0x600fbbec`, `0x600fbc3a`) are genuine, distinct, complete 22-byte functions (real `push {r7}`…`bx lr` prologue/epilogue each), not another spurious-split artifact. Independently re-derived `bruce-decompile-status.md`'s totals via a fresh header-parsed join (3,564 functions / 525,148 bytes, 0 mismatches, 0 duplicates, 0 non-census addresses) — matches the wave's claimed 1,148-byte increment and 73.48% exactly; no header slip this time.

**Two findings in this wave's own table**: (1) `0x600fba06`'s claimed failure-status name `SMP_PAIR_FAIL_CONFIRM_VALUE` for status `0x0b` is wrong — real Bluetooth SMP Pairing Failed reason code `0x0B` is **DHKey Check Failed**, not Confirm Value Failed (`0x04`), directly contradicted by the sibling `0x600fb53e` two sessions ago which correctly uses `4` for its own (legacy) confirm-value failure. This casts real doubt on the function's own "verify SC commitment" framing — it may actually be verifying the DHKey Check value instead. (2) `0x600fbb14`'s claim that its callee `0x600c1548` "starts passkey round crypto" is wrong — that address is already independently established as a generic FSM state-transition helper (`smp_set_state_phase3_enc`, 3 callers, only 1 of them this wave's function) that just requests HCI LE-Rand entropy and sets a fixed state value; no round-specific crypto anywhere in it. Both corrected in place above.

**A larger finding reaching two OLDER, pre-existing `bruce-bta-stack.md` entries**: this wave's `aes_cmac_generate_subkeys` (`0x600fbcf8`) cites `0x600c0930` as its subkey-derivation callee, and `0x600c0930` was already independently established (an earlier session) as **`smp_derive_link_key`** — "cross-transport key derivation helper (BR/EDR link key <-> LE LTK)." Reading `0x600c0930`'s own decompiled body directly (it already existed, just hadn't been cross-checked against this wave's fresh context) shows it is instead a bit-exact implementation of RFC 4493's CMAC subkey-generation algorithm: shift L left by 1 (`0x600fbc9a`), conditionally XOR the Rb constant on MSB set (`0x600fc9fc`) to produce K1, repeat on K1 to produce K2 — no link-key/cross-transport logic anywhere, and its sole caller is this wave's own generic-CMAC function (itself called only from the already-established generic top-level AES-CMAC engine used by both `BTM_BleDataSignature` and `g2` numeric comparison, ruling out a link-key-specific role). Following the chain one hop further, `0x600c0930`'s own callee `0x600c08c4` — established as **`smp_set_key_distribution`**, "calculates initiator/responder key distribution bitmasks" — is *also* wrong: its decompiled body is a bit-exact match to RFC 4493 CMAC step 6 (choose K1 for a complete final block, or pad via this wave's own confirmed `0x600fbc50`/`aes_cmac_pad` then use K2 for an incomplete one), taking K1/K2 as its two parameters. Both corrected in place in `bruce-bta-stack.md` with full evidence — real identities are internal RFC 4493 CMAC engine steps (`aes_cmac_generate_subkeys_core` / `aes_cmac_prepare_last_block`), not SMP-specific link-key or key-distribution logic. This closes the loop on the whole CMAC engine found piecemeal across this session and session 102's earlier, unrelated confirmation of the same RFC 4493 doubling step in the separate BoringSSL crypto cluster — two independent CMAC implementations in this firmware, now both correctly identified.

**Softer, hedged observation (not corrected)**: `0x600fbc9a`'s row claims a "128-bit **big-endian** bitwise left shift." Working the carry-propagation arithmetic by hand (ascending loop, `out[i] = carry_from(in[i-1]) | (in[i]<<1)`) shows this computes the correct formula for a **little-endian** 128-bit left shift (where byte 0 is least-significant) — a genuine big-endian shift needs `out[i] = (in[i]<<1) | (in[i+1]>>7)`, using the *next* (less-significant) byte's top bit, not the *previous* one's. This doesn't affect the overall "RFC 4493 subkey shift" identification (now doubly confirmed by the `0x600c0930`/`0x600c08c4` findings above), only the specific endianness descriptor — and it's possible this SMP module stores its 16-byte AES blocks byte-reversed relative to the standard convention throughout, which would make the code correct as written; that would need a look at the AES engine's (`0x600c0ac4`) own I/O byte order to settle, which is out of scope this pass. Flagged for whoever next touches this cluster rather than corrected outright.
...
| `0x600fc302` |  126 | BTM / SMP | **`smp_generate_ltk`** — Broadcom SMP generate Long Term Key (LTK) via AES (`0x6009ab94` / `0x600fbc02`), mask to key size (`0x600fc9c4`) and store it at `+0x1c6`, then ~~save LTK record (`0x600c0ed4`)~~ ⚠️ **wrong, corrected QA session 148** — `0x600c0ed4` is already independently established (`bruce-bta-stack.md`) as `smp_set_state_phase2_check`, and its own decompiled body (read in full) confirms it exactly: sets the FSM sub-state field `+0x1f2` to a fixed value (`5`, matching "sub-state 5") and requests one block of `btsnd_hcic_ble_rand` entropy — no record-save logic anywhere in it, byte-for-byte the same shape as last session's already-corrected `0x600c1548` finding. The actual LTK persistence already happened via the memcpy to `+0x1c6` immediately before this call; the real effect of this call is "advance the FSM to the next pairing sub-state and request fresh entropy," not "save record.". | 2 callers / 6 callees |
...
**⚠️ QA session 148 note:** Backbone: all 14 addresses/sizes match the census exactly, fully contiguous `0x600fbe82`–`0x600fc438`. Independently re-derived `bruce-decompile-status.md`'s totals via a fresh header-parsed join (3,578 functions / 526,610 bytes, 0 mismatches, 0 duplicates, 0 non-census addresses) — matches the wave's claimed 1,462-byte/73.69% exactly, no header slip.

**Confirmed finding, the same recurring pattern as last session**: `smp_generate_ltk`'s claim that `0x600c0ed4` "saves LTK record" is wrong — see the corrected row above. This is the *second consecutive session* a wave has mistaken a generic "set FSM sub-state N, request LE-Rand entropy" helper (this one, and last session's `0x600c1548`) for something specific to its own narrative context (last session: "passkey round crypto"; this session: "save record") — worth flagging explicitly to future waves as a recognizable shape (a ~40-byte function, single state-byte write at `+0x1f2`, one `btsnd_hcic_ble_rand` call, one `FUN_600c15a8(0)` error path) that is *never* a save/persist/crypto step on its own.

**Strong positive corroboration**: traced `smp_c1_step1`/`smp_c1_step2` (`0x600fc110`/`0x600fc17a`) end to end and confirmed the two-step c1 algorithm structure matches the real Bluetooth LE Legacy Pairing confirmation-value function exactly at the algorithmic level — step 1 XORs the freshly-built p1 block against `r` (via the confirmed generic XOR helper `0x600fc9fc`) then encrypts with the confirmed `smp_crypto_encrypt_wrapper` using the key at `+0x1b6`; step 2 does the identical XOR-then-encrypt with p2 against step 1's result, i.e. `e(TK, e(TK, r XOR p1) XOR p2)`, the textbook `c1` formula.

**Softer, hedged observation (not corrected), continuing last session's noted byte-order ambiguity**: `smp_build_c1_p1_params`/`smp_build_c1_p2_params`'s claimed field orders (`pres||preq||rat||iat`, `padding||ia||ra`) name the right fields, but the actual write order in the code places the address-type bytes/padding at the *opposite* end from where the spec's literal left-to-right reading would put them (address-type bytes written first for p1; the memset-derived zero padding left at the *end*, not the *start*, for p2 — the two 6-byte address blocks are written first). Since LE Legacy Pairing's `c1` must interoperate with real, spec-compliant peer devices for pairing to succeed at all (and Stadia controllers demonstrably do pair with off-the-shelf BLE hosts), the underlying computation is almost certainly still spec-correct end-to-end — this reads as a continuation of last session's noted big-endian/little-endian buffer-convention question in this same SMP crypto cluster (possibly a consistently-reversed internal storage order that self-cancels through the encrypt step), not a functional bug. Flagged for whoever next has reason to trace this cluster's exact byte conventions rather than corrected outright. Also noted, not chased further (per the established sessions-53–55 precedent for this artifact class): `0x600fc110`'s own decompiled signature shows only 2 parameters, but both its callers (`0x600fc206`, `0x600fc232`) pass 5 arguments — a real callee-under-typing, but the extra arguments aren't referenced anywhere in the body I read, so the c1-step1 tracing above is unaffected.
...
| `0x600fc542` |  192 | Crypto / SMP | **`smp_crypto_f4_confirm`** — Broadcom SMP Secure Connections cryptographic confirm function `f4`: computes 16-byte confirm value via `AES-CMAC_X(U \|\| V \|\| Z)` ⚠️ **byte order corrected QA session 149 — see note below: actual write order is `Z \|\| V \|\| U`, the exact reverse of the spec's literal notation** over 65-byte payload (`0x600c09f0`), field sizes exactly matching spec (U=32, V=32, Z=1). | 3 callers / 1 callee |
| `0x600fc602` |  342 | Crypto / SMP | **`smp_crypto_f5_kdf`** — Broadcom SMP Secure Connections key generation function `f5`: computes MacKey and LTK via `AES-CMAC_T(Counter \|\| keyID \|\| N1 \|\| N2 \|\| A1 \|\| A2 \|\| Length)` ⚠️ **byte order corrected QA session 149 — see note below: actual write order is the complete reverse, `Length \|\| A2 \|\| A1 \|\| N2 \|\| N1 \|\| keyID \|\| Counter`, all 7 fields** over 53-byte payload (`0x600c09f0`), field sizes exactly matching spec (Counter=1, keyID=4, N1=16, N2=16, A1=7, A2=7, Length=2). | 1 caller / 1 callee |
...
| `0x600fc870` |  314 | Crypto / SMP | **`smp_crypto_f6_dhkey_check`** — Broadcom SMP Secure Connections DHKey check function `f6`: computes 16-byte check value via `AES-CMAC_W(N1 \|\| N2 \|\| R \|\| IOcap \|\| A1 \|\| A2)` ⚠️ **byte order corrected QA session 149 — see note below: actual write order is `A2 \|\| A1 \|\| IOcap \|\| R \|\| N2 \|\| N1`, the exact reverse of the spec's literal notation** over 65-byte payload (`0x600c09f0`), field sizes exactly matching spec (N1=16, N2=16, R=16, IOcap=3, A1=7, A2=7). | 2 callers / 1 callee |
...
**⚠️ QA session 149 note:** Backbone: all 15 addresses/sizes match the census exactly, including the Fix #81 restoration and the `0x600fc4d8` header-comment-only touch (its body is byte-identical to before, confirmed via `git show`, so no re-verification of its content was skipped). Independently re-derived `bruce-decompile-status.md`'s totals via a fresh header-parsed join (3,592 functions / 528,432 bytes, 0 mismatches, 0 duplicates, 0 non-census addresses) — matches the wave's claimed 1,928-byte/73.94% exactly.

**The f4/f5/f6 payload sizes are an exceptionally strong, exact match to the real Bluetooth Core Spec LE Secure Connections cryptographic toolbox** — read all three functions in full and hand-summed every field: `f4`'s message is `U`(32B)+`V`(32B)+`Z`(1B) = 65 bytes, `f5`'s is `Counter`(1)+`keyID`(4)+`N1`(16)+`N2`(16)+`A1`(7)+`A2`(7)+`Length`(2) = 53 bytes, `f6`'s is `N1`(16)+`N2`(16)+`R`(16)+`IOcap`(3)+`A1`(7)+`A2`(7) = 65 bytes — every single field size matches the spec exactly, and the three total payload lengths (65/53/65) match the wave's own claimed byte counts exactly. This is one of the strongest spec-level confirmations in this pipeline's history.

**But a consistent, now-confirmed (not merely suspected) finding**: the actual byte-write order inside all three functions is the *complete reverse* of the spec's literal field notation asserted in each row — most conclusively in `f5`, where all 7 fields reverse perfectly (code order `Length→A2→A1→N2→N1→keyID→Counter` is the exact mirror of the spec's `Counter→keyID→N1→N2→A1→A2→Length`), and equally cleanly in `f4` (`Z→V→U` vs. spec's `U→V→Z`) and `f6` (`A2→A1→IOcap→R→N2→N1` vs. spec's `N1→N2→R→IOcap→A1→A2`). This is the third consecutive session (147's `aes_cmac_shift_left_128`, 148's `c1` p1/p2 field order, now this session's f4/f5/f6) to surface the same directional pattern in this SMP crypto cluster, and with three fields reversing in perfect lockstep this session, it's no longer reasonable to treat it as an open question — **this SMP module consistently constructs its AES-CMAC message buffers in the exact reverse of the spec's left-to-right field notation**, almost certainly a deliberate, self-consistent internal storage convention (this is LE Secure Connections cryptography that must interoperate bit-exactly with real, spec-compliant BLE peers to pair at all — and Stadia controllers demonstrably do — so the actual computed values are essentially certain to be spec-correct; only the *literal in-memory field order asserted by these table rows' formula notation* was backwards). Corrected all three rows in place; recommend this be treated as a standing fact about this whole cluster going forward rather than re-flagged function by function.

**A real correction reaching an OLDER, pre-existing row**: session 146's `btm_sec_process_key_missing` (`0x600fb9d2`) claimed it "resets crypto state (`0x600fc4d8`)" — now that this wave gives `0x600fc4d8` a real identity (`smp_sc_calculate_dhkey`, a genuine P-256 EC scalar-multiplication computation), that claim is directly contradicted: computing a DH shared secret is not a state reset. Corrected in place, and softened the enclosing function's own "link key missing" framing to unconfirmed, since starting with an SC-specific DHKey recomputation is an odd fit for that scenario.

**Other functions spot-checked and confirmed clean**: `smp_xor_128`'s in-place 16-byte XOR loop matches exactly; `smp_mask_enc_key_size`'s zero-fill-from-key-size-to-16 loop matches precisely and is now independently confirmed as the real mechanism behind the "mask to key size" claims made repeatedly in sessions 147–148; `smp_select_pairing_method`'s AuthReq bit checks (`0x08`/`0x10`) match the real Bluetooth AuthReq bitmask exactly (bit 3 = SC, bit 4 = Keypress per spec); `smp_reset_sec_record`'s 516-byte clear with two preserved fields matches its own literal offsets exactly.
...
**⚠️ QA session 150 note:** Backbone: all 15 addresses/sizes match the census exactly, fully contiguous `0x600fcc88`–`0x600fd0f2`. Independently re-derived `bruce-decompile-status.md`'s totals via a fresh header-parsed join (3,607 functions / 529,562 bytes, 0 mismatches, 0 duplicates, 0 non-census addresses) — matches the wave's claimed 1,130-byte/74.10% exactly.

**A real, well-evidenced correction reaching an OLDER `bruce-bta-stack.md` entry**: `smp_release_l2cap_buffer`'s claim that `0x600b542c` "releases the L2CAP channel 6 (SMP) security context buffer" directly contradicted that address's established identity, `l2c_ble_link_sec_comp` — "BLE link security completion event router to active CCBs." Read `0x600b542c`'s own decompiled body in full to settle it: it validates a CID parameter (must be 4–7), looks up the link's LCB by BD_ADDR via the already-confirmed `l2cu_find_lcb_by_bd_addr`, retrieves and nulls the fixed-channel CCB pointer stored for that specific CID, and hands the pointer to the already-confirmed `l2cu_release_ccb` — a targeted "find and free one fixed channel's CCB" operation, with no event-dispatch or multi-CCB routing logic anywhere. This wave's own framing turns out to be the accurate one; the established doc entry was wrong. Corrected in `bruce-bta-stack.md` with full evidence. This also retroactively confirms session 131's own soft caveat about this same address (cited there for CID 4/ATT teardown in `gatt_disconnect_transport_channel`) was on the right track — both callers (this wave's CID 6/SMP and session 131's CID 4/ATT) are genuine teardown contexts, consistent with the corrected identity.

**Strong spec/cross-session corroboration elsewhere**: `smp_sc_extract_passkey_round_bit`'s `bit | 0x80` formula matches the real Bluetooth SC Passkey Entry `Z` value definition exactly (`Z = 0x80` or `0x81` depending on the passkey bit); `smp_pack_local_iocap_3b`/`smp_pack_peer_iocap_3b` read the *exact same three struct offsets* (`0x18e`/`400`/`0x192` and `0x18d`/`399`/`0x191`) that session 148's `smp_pack_local_pairing_params`/`smp_pack_peer_pairing_params` used for the IOcap/OOB/AuthReq fields of the 7-byte Pairing Request/Response PDU — a precise, independent cross-session field-offset match, since f6's IOcap parameter really is defined as a subset of those same three PDU fields. `smp_format_local_addr_7b`/`smp_format_peer_addr_7b`'s "reversed BD_ADDR + AddrType" claim is a further data point for the standing reversed-buffer-convention finding from sessions 147–149 — this time a genuine intra-field byte reversal of the 6-byte address itself, with the address-type byte appended at the end rather than prepended as the spec's literal `AddrType || Device Address` order would suggest; consistent with, not contradicting, the established pattern.
...
| Address | Bytes | Subsystem | Functional Role & Evidence | Call graph |
|---|---:|---|---|---|
| `0x600fd0f2` |   30 | BTM / Security | **`btm_sec_send_auth_fail`** — Broadcom BTM ~~send security authentication failure status 0x0B to BTM layer via `0x600a4a20`~~ ⚠️ **mischaracterized, corrected QA session 151** — `0x600a4a20`'s own decompiled body (already established as BTM's SSP user-confirmation accept/reject handler) shows its first parameter is used *only* as a boolean reject flag (`local_11 = param_1; ...; if (local_11 == 0) {accept-path} else {reject-path}`) — the literal value `0xb` never propagates anywhere as a transmitted status/reason byte, it's purely "nonzero = reject." Real effect: reject the pending SSP user confirmation for this device, not "send status 0x0B.". | 0 callers / 1 callee |
...
| `0x600fd276` |   62 | BTM / Security | **`btm_sec_confirm_link_status_1`** — Broadcom BTM ~~confirm link status (success 0 or fail 0x0B) to security manager via `0x600f068e`~~ ⚠️ **mischaracterized, corrected QA session 151** — `0x600f068e` is already independently established (session 116) as a thin BTA-layer wrapper around `SMP_ConfirmReply`, not a "link status" reporting function. Real effect: forward an accept(0)/reject(nonzero) decision into SMP's Confirm-Value user-reply API, part of the pairing flow, not ACL/HCI link status. | 0 callers / 1 callee |
| `0x600fd2b4` |   50 | BTM / Security | **`btm_sec_confirm_link_status_2`** — Broadcom BTM ~~confirm link status (success 0 or fail 0x0B) to security manager via `0x600f06dc`~~ ⚠️ **mischaracterized, corrected QA session 151** — `0x600f06dc` is already independently established (session 116) as a thin BTA-layer wrapper around `SMP_NumericComparisonReply`, not a "link status" reporting function. Real effect: forward an accept(0)/reject(nonzero) decision into SMP's Numeric Comparison user-reply API. | 0 callers / 1 callee |
| `0x600fd2e6` |   34 | BTM / Security | **`btm_sec_set_encryption_state`** — Broadcom BTM ~~update device encryption state via `0x600f0662`~~ ⚠️ **mischaracterized, corrected QA session 151** — `0x600f0662` is already independently established (session 116) as a thin BTA-layer wrapper around `SMP_PasskeyReply`, not an encryption-state updater. This is the third member of session 116's already-confirmed `0x600f0662`/`0x600f068e`/`0x600f06dc` = `SMP_PasskeyReply`/`SMP_ConfirmReply`/`SMP_NumericComparisonReply` trio, and this wave's own three callers of that trio (`0x600fd276`, `0x600fd2b4`, this row) are all mischaracterized the same way — see the QA note below. | 0 callers / 1 callee |
...
**⚠️ QA session 151 note:** Backbone: all 15 addresses/sizes match the census exactly, including the Fix #82 restoration; fully contiguous `0x600fd0f2`–`0x600fd328`. Independently re-derived `bruce-decompile-status.md`'s totals via a fresh header-parsed join (3,622 functions / 530,128 bytes, 0 mismatches, 0 duplicates, 0 non-census addresses) — matches the wave's claimed 566-byte/74.17% exactly. All 15 of this wave's own functions show 0 callers — consistent with the rest of this address range being a handler/dispatch-table cluster reached only through unresolved indirect calls, matching the pattern already seen throughout this stretch of the census.

**Four related findings, all one underlying pattern**: this wave's own callees repeatedly get mischaracterized as "status reporting" functions when their established (or directly-read) real behavior is something else. (1) `0x600fd0f2`'s claim that it "sends status 0x0B" via `0x600a4a20` — read that callee's own body (already established as BTM's SSP user-confirmation accept/reject handler) directly: its first parameter is used *only* as a boolean reject flag, never transmitted as a value; passing `0xb` has the identical effect to passing `1`. (2)/(3)/(4) `0x600fd276`, `0x600fd2b4`, and `0x600fd2e6` each claim their callee (`0x600f068e`/`0x600f06dc`/`0x600f0662` respectively) reports "link status" or "encryption state" — but that exact trio of addresses is already independently established (session 116, unhedged) as thin BTA-layer wrappers around `SMP_ConfirmReply`/`SMP_NumericComparisonReply`/`SMP_PasskeyReply` — genuine SMP pairing user-response APIs, with no ACL-link-status or encryption-state semantics at all. All four corrected in place. By contrast, `0x600fd1ec` (`btm_sec_forward_auth_status`) — which forwards *variable*, struct-sourced arguments through to `0x600a4a20` rather than a hardcoded literal — was left uncorrected, since its more generic "forward auth status and key data" framing isn't specifically contradicted the way the other four rows' precise "0x0B"/"link status" claims are.

**Softer, unflagged observation**: `0x600fd14a` (`btm_sec_stub_return_0xb`) is a bare 24-byte "return 0xb" stub with no visible caller — its row's "Auth Fail" gloss on the constant is speculative (inferred from the neighboring `0x600fd0f2`, which this session's finding shows uses `0xb` only as a generic truthy reject flag, not a specific reason code), but with zero caller context there's no way to confirm or refute it either way; left as-is rather than corrected.
...
**⚠️ QA session 152 note:** Backbone: all 15 addresses/sizes match the census exactly, fully contiguous `0x600fd328`–`0x600fd60e`. Independently re-derived `bruce-decompile-status.md`'s totals via a fresh header-parsed join (3,637 functions / 530,870 bytes, 0 mismatches, 0 duplicates, 0 non-census addresses) — matches the wave's claimed 742-byte/74.28% exactly.

**Strong corroboration, zero corrections needed on this wave's own claims**: `bta_dm_ble_set_adv_or_scan_rsp_data_handler`'s cited opcodes `0x2008`/`0x2009` for its two callees (`0x6009cec8`/`0x6009ce30`) match a real, disassembly-confirmed HCI LE opcode pair — this exact pair was independently corrected onto these two addresses by session 30's QA pass, and this wave's fresh citation matches that established finding precisely. `bta_dm_ble_set_conn_params_handler`, `bta_dm_ble_update_conn_params_handler`, and `bta_dm_ble_set_adv_config_handler`'s callees (`0x600f0726`, `0x600f7b48`, `0x6009cc7c`) all match their established identities (`bta_dm_ble_set_conn_params`, `L2CA_UpdateBleConnParams`, `BTM_SetAdvParams`) exactly, with argument shapes consistent across sessions.

**One unresolved connection worth flagging for a future session, not a correction**: all 8 "dispatch event N" functions (`0x600fd464`–`0x600fd5da`, events 5 through 12) call `0x60095d9c`, whose own established `bruce-bta-stack.md` entry is part of the exploratory, session-30-corrected "not H4 UART transport" cluster — described there only loosely as "an event-ID (`0x1e00`–`0x1e10`) constructor/dispatcher pair plus generic multi-slot listener register/notify helpers," with 11 callers and 0 callees, and no decompiled body of its own yet. This wave's 8 new call sites give much more concrete shape to that vague description (small literal event codes 5–12, not the `0x1e00`+ range the sibling constructor half uses, plus specific small payload sizes per event) — plausibly consistent with `0x60095d9c` being the generic "notify" half of that pair, but this can't be confirmed without decompiling `0x60095d9c` itself. Not flagged as a contradiction, since nothing here directly conflicts with the established (already-hedged) description — noted as a good target for whoever next has reason to fully resolve that cluster.
...
**⚠️ QA session 153 note:** Backbone: all 12 addresses/sizes match the census exactly, fully contiguous `0x600fd60e`–`0x600fdc46`. Independently re-derived `bruce-decompile-status.md`'s totals via a fresh header-parsed join (3,649 functions / 532,462 bytes, 0 mismatches, 0 duplicates, 0 non-census addresses) — matches the wave's claimed 1,592-byte/74.50% exactly.

**A significant correction to a several-sessions-old, previously "(QA-verified)"-tagged `bruce-bta-stack.md` entry**: this wave's own 4 fresh call sites into `0x60094d24` (`bta_gattc_open_connection`, `bta_gattc_process_api_open_bg`, `bta_gattc_process_api_open`, `bta_gattc_process_api_cancel_open`) all pass literal `0x1Dxx`-range values as its second argument — this doc's own established convention for BTA GATTC event codes (`0x1d08`, `0x1d13`, `0x1d15`, etc.) — directly at odds with `0x60094d24`'s established identity, `BTM_SetPowerMode`, whose real second parameter would need to be a tiny power-mode enum (Active/Sniff/Park/Hold), never a `0x1D0D`-class value. Read `0x60094d24`'s own decompiled body directly to settle it (it already existed, just hadn't been cross-checked against this wave's fresh context): it's a textbook generic 2D `[state][event]` finite-state-machine dispatcher — looks up the current state byte, indexes a transition table by `(state, event)` to get the next state and one action ID, writes the new state back, and invokes a single action handler through a function-pointer table. No HCI command, no power-mode logic anywhere. This is the same class of pattern as this doc's own `state_machine.cc`/`bta_sys_sm_execute` entries, and the previously-puzzling "0 callees" is explained (not contradicted) by the sole call being an unresolved indirect function-pointer dispatch. Corrected `bruce-bta-stack.md` in place with full evidence; this wave's own rows needed no changes, since their "dispatch event via `0x60094d24`" framing was already the accurate one — it was the *older* entry that was wrong. Given the previous entry carried a "(QA-verified)" tag, this is a useful reminder that such tags mark a specific past verification pass, not a permanent guarantee — a later wave's fresh, concrete calling-context evidence can still surface a miss.

**Other citations spot-checked and confirmed clean**: `gatt_connect_dev`/`gatt_disconnect_dev` (`0x600f3c42`/`0x600f3caa`) both match their established identities exactly, including argument shapes; `bta_gattc_send_cancel_open_cback_error`/`bta_gattc_process_api_cancel_open_bg`'s shared "event 0x0E + BTA_GATT_ERROR (`0x85`)" callback pattern reads directly and consistently from both bodies.
...
**⚠️ QA session 155 note:** Backbone: all 11 addresses/sizes match the census exactly, fully contiguous `0x600fe2fe`–`0x600fe960`. Verified both Fix #84/#85 restorations directly via disassembly: `0x600fe88c` is a genuine, complete 22-byte no-op stub (real `push{r7}`…`bx lr`) and `0x600fe8a2` is a genuine, complete 116-byte function with its own real prologue/epilogue — no bleed-through from either address. Independently re-derived `bruce-decompile-status.md`'s totals via a fresh header-parsed join (3,670 functions / 535,782 bytes, 0 mismatches, 0 duplicates, 0 non-census addresses) — matches the wave's claimed 1,634-byte/74.95% exactly.

**A well-evidenced correction to an older entry**: `bta_gattc_q_cmd`'s callee `0x600ef476` is already established as `bta_gattc_register_notify_filter` — but this wave's own `bta_gattc_write_char_value`, `bta_gattc_execute_write`, and `bta_gattc_q_cmd` (plus session 154's `bta_gattc_start_service_discovery`, `bta_gattc_read_char_by_uuid`, `bta_gattc_read_multiple`) all call this exact address with the identical 2-argument pattern as the very first gate of their own, completely unrelated operations — read directly, its body is a generic "claim the single pending-operation slot if empty" guard (checks `+0x14` for null, stores the caller's own second argument there, sets a flag bit at `+0x18`), with no notification-specific logic anywhere. Each of these 6+ callers stores its *own* incoming event-data pointer, not a notification filter callback. This also lines up with the already-corrected (session 112) description of the same `+0x14` field on the neighboring `bta_gattc_free_clcb` as a generic "pending GATT command" slot. Corrected `0x600ef476`'s entry in place — real identity is a generic pending-command-slot guard used across essentially the whole GATTC client action-table dispatch layer, not notification-filter-specific.

**A positive resolution of a previously "left unconfirmed" entry**: `bta_gattc_execute_write`'s callee `0x600f3ade` was explicitly left "unconfirmed" by session 128's QA after that session refuted its original `gattc_search_services` identity (its own callee, `0x600f46b4`, was found to send an ATT Execute Write Request, not a discovery request). This wave's `bta_gattc_execute_write` calls `0x600f3ade` specifically to execute/cancel a queued reliable write — independently converging with session 128's opcode-level finding from the opposite direction (caller intent vs. callee opcode). Updated `0x600f3ade`'s entry to reflect this: real identity is very likely `GATTC_ExecuteWrite`, no longer unconfirmed.

**Strong corroboration elsewhere**: every `0x60094d24` action-table dispatch this wave makes (`0x1d10`×3) continues to match the corrected generic-dispatcher identity from sessions 153–154; `bta_gattc_write_cmpl_handler`'s three-way event selection (4/9/0x0B) and `bta_gattc_read_cmpl_handler`'s two-way selection (3/8) both match their own decompiled branch logic exactly; `bta_gattc_cfg_mtu_cmpl_handler`'s MTU-field write and event `0x11` match precisely; four of this wave's functions correctly reuse `0x600aa648` (the session-112-corrected `gatt_release_sr_cmd`) for completion-handler cleanup, consistent with that correction.
...
**⚠️ QA session 156 note:** Backbone: all 12 addresses/sizes match the census exactly (two small gaps — 34 bytes and 22 bytes — confirmed to hold zero census entries, genuine undecompiled regions). Independently re-derived `bruce-decompile-status.md`'s totals via a fresh header-parsed join (3,682 functions / 537,416 bytes, 0 mismatches, 0 duplicates, 0 non-census addresses) — matches the wave's claimed 1,634-byte/75.18% exactly.

**A correction reaching a long-stale `bruce-bta-stack.md` entry, resolving one of session 114's explicitly-flagged-but-unverified addresses**: `bta_gatts_deregister_if_active`'s callee `0x60095b2c` is already established as part of a session-30 "not H4 UART, event-ID/listener cluster" correction — but that correction text was itself already flagged by session 114 as one of six addresses sharing an identical, mechanically-copy-pasted boilerplate paragraph whose specific cited evidence was only ever independently checked for one member (`0x600959cc`); `0x60095b2c` was one of the five left unverified. Read its own decompiled body directly this session: a plain 10-entry, 32-byte-stride linear table search (active-flag check + key-byte match at offset `0x1c`) — matching neither the original H4-UART claim nor the session-30 "event-ID/listener" replacement, and instead matching this same wave's own `bta_gatts_deregister_all` (`0x600fef54`), which independently iterates the identical 10-entry/`0x20`-byte-stride table. Real identity: a GATTS server-control-block find-by-ID lookup, corrected in `bruce-bta-stack.md` with a pointer flagging that the remaining 4 unverified addresses from session 114's group shouldn't be assumed correct either, since this one matched *neither* prior narrative. Also added a correction note to the older, already-flagged-as-unreliable "Key Architectural Discoveries" prose section, which independently made the same (now-refuted) claim about this address in more specific language.

**Other citations spot-checked and confirmed clean**: `bta_gattc_process_notification`'s `0x600f3b78` (confirm indication) matches its established `gattc_mtu_confirmation_cb`-adjacent role; `bta_gattc_sdp_cmpl_handler`'s `0x60094c5c` matches the established `bta_gattc_rebuild_cache`; this wave's four more `0x60094d24` dispatch calls (`0x1d00`, `0x1d0d`) continue to match the corrected generic-dispatcher identity from sessions 153–155.
...
**⚠️ QA session 159 note:** Backbone: all 12 addresses/sizes match the census exactly, fully contiguous `0x600ffc9e`–`0x60100324`. Independently re-derived `bruce-decompile-status.md`'s totals via a fresh header-parsed join (3,718 functions / 542,356 bytes, 0 mismatches, 0 duplicates, 0 non-census addresses) — matches the wave's claimed 1,670-byte/75.87% exactly.

**Given this crypto cluster's history of misidentifications (sessions 90–106), gave every claim in this wave the full Rule-9 defining-behavior treatment rather than trusting the plausible naming — and it holds up unusually well, anchored by two independently-confirmed primitives from many sessions ago.** `aes_encrypt_block`'s structure was traced precisely: `AddRoundKey(round 0)` → loop of `[round transform via 0x600c80fc] + [AddRoundKey]` for rounds `1..N-1` → `[final round transform via 0x600c7fc8] + [AddRoundKey(round N)]` — the textbook AES structure (final round omits MixColumns). Critically, `0x600c7fc8` is not a fresh guess: it's already independently established, "spec-verified byte-for-byte," as AES SubBytes+ShiftRows — and this wave correctly places it in the *final*-round position, exactly where real AES's asymmetric last round belongs. The caller, `0x600c0ac4`, is the already-confirmed `smp_encrypt_data` — a real AES-128 primitive being called by SMP's own confirmed encryption wrapper is precisely the expected shape.

**The ECC point-arithmetic cluster is similarly well-anchored, not merely plausible-sounding**: every one of `ec_point_init`/`ec_point_copy`/`ec_point_double`/`ec_point_add`/`ec_scalar_compute_naf`/`ec_bignum_zero` is called directly by `0x600c8798` — already independently established (session 7, reinforced through session 149's `smp_sc_calculate_dhkey`) as the confirmed windowed-NAF EC scalar-multiplication engine. `ec_point_double`/`ec_point_add` both operate on 96-byte `(X,Y,Z)` structs at consistent `+0`/`+0x20`/`+0x40` offsets and compose `0x601005ac`/`0x601005f6` — already independently identified (session 7's "P-256/P-192 modular-reduction helpers") — alongside `0x600c89e0`/`0x600c8a54`/`0x600c8acc` (plausibly square/add/mul), each producing exactly one new field value per XYZ output slot, consistent with genuine Jacobian doubling/addition formulas (this pipeline hasn't independently confirmed each individual field-op's exact identity, so the specific "which callee is square vs. mul vs. add" mapping is not separately verified — only the overall structural shape). `ec_scalar_compute_naf`'s loop — check-bottom-bit, branch on the low 2 bits (0/1/3, with the `3` case incrementing the scalar with carry-propagation to force an even value) then halving every iteration, packing 2-bit signed digits — is the real, textbook NAF-computation algorithm, not a generic-looking guess. `ec_bignum_cmp`'s highest-limb-first comparison loop matches a standard big-number magnitude-compare exactly.
...
**⚠️ QA session 160 note:** Backbone: all 18 addresses/sizes match the census exactly (a 4-byte gap between `0x601004b2`-derived range and `0x601007b6` confirmed to hold zero census entries, likely alignment padding). Independently re-derived `bruce-decompile-status.md`'s totals via a fresh header-parsed join (3,736 functions / 543,942 bytes, 0 mismatches, 0 duplicates, 0 non-census addresses) — matches the wave's claimed 1,586-byte/76.09% exactly.

**`ec_field_mul`'s "P-256 / P-192" claim is precisely, numerically confirmed, not just plausible** — this wave finally decompiles the actual function last session's QA note cited only by reputation (as an already-established "P-256/P-192 modular-reduction helper" from session 7). Read its body directly: it computes a full double-width schoolbook product (`ec_bignum_mul`) then branches on the word-count parameter — `param_4 == 6` (6×32=**192** bits) calls one reduction function, `param_4 == 8` (8×32=**256** bits) calls a different one. This is an exact, checkable numeric match to the two specific curve sizes named in the claim, not a naming coincidence — about as strong a confirmation as a bignum-layer function gets. All of this wave's other bignum primitives (`ec_bignum_add`/`_sub`/`_lshift1`/`_rshift1`/`_mul`/`_is_zero`/`_word_length`/`_bit_length`) were read in full and match textbook multi-limb arithmetic algorithms exactly (ripple-carry add/subtract, correct little-endian shift-with-carry direction in both shift functions, standard `O(n²)` schoolbook multiply with 64-bit partial products) — and `ec_bignum_mul` correctly zeroes its output via `ec_bignum_zero`, the exact function session 159 independently confirmed.

**RTOS cluster spot-checked and consistent, though less independently verifiable**: `rtos_timer_clear_active`'s only two callers are `0x600ca0fc` and the already-established, string-named `timers__600cad24` (`timers.c`), supporting the timer framing; `rtos_queue_create` calls the already-established `queue__600c9de4` (`queue.c`); `rtos_task_create`'s allocation calls `thunk_EXT_FUN_00007f58`, already independently identified (session 66) as the `pvPortMalloc` free-list search, matching "dynamically allocate" precisely. The three generic callback-walking utilities (`rtos_check_callback_table_4`/`_validate_struct_members`/`_validate_circular_list`) show a genuine circular-linked-list traversal with wraparound-sentinel detection in the last one, consistent with their claims, though not independently cross-referenced against an established citation.
...
| `0x60100b48` |   34 | Flash / KVS | ~~**`kvs_calc_and_store_crc`** — Key-Value Store calculate and store sector checksum / CRC.~~ ⚠️ **wrong, corrected QA session 161** — its callee `0x60050c18` is already independently established, dozens of times over, as the generic "format a raw value into a diagnostic/log buffer" helper (`json_format_metric_u32`, `log_msg_format_hexdump`, `state_machine_format_field_1`/`_2`, `gatt_server_format_uint16`, and ~20 more call sites, all using the identical `(value, mode, dest_ptr, remaining_capacity)` argument pattern this function also uses) — session 83 already refuted the exact same "CRC via `0x60050c18`" claim for a different function (`power_rpcs_buffer_crc_verify`). This wave's own `kvs_hash_key_value` (`0x60100c14`, below) correctly identifies the *real* CRC routine in this cluster (`0x600cc29c`, independently confirmed elsewhere as a CRC-16/CCITT-style function seeded with `0xffff` — and this function's own body opens with exactly that literal), sharpening the contrast: this row's callee is the wrong one. Real behavior is very likely formatting/logging a diagnostic value about the sector, not computing a checksum. | 1 caller / 2 callees |
...
**⚠️ QA session 161 note:** Backbone: all 24 addresses/sizes match the census exactly, fully contiguous `0x6010095a`–`0x60100f90`. Independently re-derived `bruce-decompile-status.md`'s totals via a fresh header-parsed join (3,760 functions / 545,532 bytes, 0 mismatches, 0 duplicates, 0 non-census addresses) — matches the wave's claimed 1,590-byte/76.30% exactly.

**Strong subsystem-level corroboration, plus one confirmed misidentification within it**: this whole KVS cluster's identity is well-anchored — 5 of its own functions (`kvs_align_up`, `kvs_flash_get_sector_header_len`, `kvs_calc_and_store_crc`, `kvs_calc_entry_size`, `kvs_hash_key_value`) are called directly by already-established, string-named `key_value_store__*` functions (`0x600cb460`, `0x600cb598`, `0x600cb6fc`, `0x600cb39c`), confirming this really is the same flash key-value-store engine, not a mislabeled subsystem. **But `kvs_calc_and_store_crc`'s specific "CRC" claim is wrong**: its callee `0x60050c18` is already independently established — across roughly two dozen already-documented call sites using the identical `(value, mode, dest, remaining_capacity)` argument pattern this function also uses — as the generic "format a raw value into a diagnostic/log buffer" helper, not a checksum routine; session 83 already refuted this exact "CRC via `0x60050c18`" claim once before, for a different function. The contrast within this same wave sharpens the point: `kvs_hash_key_value` (`0x60100c14`) correctly identifies the *real* CRC engine in this cluster, `0x600cc29c` — already established elsewhere as a CRC-16/CCITT-style routine seeded with `0xffff` — and this function's own body opens with exactly that literal seed. Corrected `kvs_calc_and_store_crc`'s row in place; real behavior is very likely a diagnostic-log formatter for sector state, not a checksum calculator. `kvs_write_record`'s "magic `0x55AA`" claim was independently verified against its own literal `0x55aa` store — exact match.
...
**⚠️ QA session 162 note:** Backbone: all 19 addresses/sizes match the census exactly, fully contiguous `0x60100f90`–`0x6010162e` once sorted by address. Verified the `0x601013d0` spurious-split merge directly via disassembly: one continuous instruction stream from the real `push.w {r4,r5,r6,r7,r8,sb,lr}` prologue through the single real epilogue (`pop.w ...,pc}`), with both claimed split points (`0x60101468`, `0x60101492`) confirmed to be mid-stream instructions reached only by fall-through/branch, no independent prologue at either. Independently re-derived `bruce-decompile-status.md`'s totals via a fresh header-parsed join (3,775 functions / 546,898 bytes, 0 mismatches, 0 duplicates, 0 non-census addresses) — matches the wave's claimed 1,694-byte/76.49% exactly.

**Strong subsystem-level corroboration throughout**: all 6 new KVS public-API functions compose the already-established, string-named `key_value_store__*` functions and last session's own confirmed low-level primitives (`0x60100c48`/`0x60100e08`/`0x60100ed8`/`0x60100bb0`/`0x60100d98`/`0x60100b38`/`0x60100e78`) consistently. `rpc_service_broadcast_msg` (`0x60101302`) — a function this project has referenced by reputation since sessions 14–15 as a "generic vtable-based transport fan-out shared by all RPC replies" — was read in full for the first time this session and matches that description exactly: iterates a registered-endpoint array, checks each via a vtable "active" call, dispatches via a vtable "send" call. The `thunk_EXT_FUN_0000b4c2`/`thunk_EXT_FUN_0000887a` pair this wave uses as a lock/unlock bracket around several functions ("thread-safe") is consistent with, not contradicted by, `bruce-itcm.md`'s lower-level finding that `0xb4c2` is `xQueueGenericReceive`-shaped — real FreeRTOS implements `xSemaphoreTake`/`Give` as thin wrappers over the queue primitives, so both descriptions (queue-receive mechanism vs. mutex-acquire usage) are correct at their respective levels, not in tension.
...
| `0x6010196e` |   22 | Diagnostics / JSON | ~~**`rpc_arg_get_type_len`** — Read RPC parameter length / type field.~~ ⚠️ **mischaracterized, corrected QA session 163** — its own body (`param_1[2]++; *(byte*)(*param_1+old_val)=param_2; *(byte*)(*param_1+param_1[2])=0;`) *appends* one byte to the buffer and null-terminates it — a WRITE, the opposite direction from "read... a field." See the cluster-wide note below the table. | 3 callers / 0 callees |
| `0x60101984` |   34 | Diagnostics / JSON | ~~**`rpc_arg_validate_header`** — Validate RPC argument descriptor header.~~ ⚠️ **mischaracterized, corrected QA session 163** — its body writes the literal ASCII bytes `0x6e,0x75,0x6c,0x6c` = `"null"`, the JSON null-value literal, not a header-validation check. See the cluster-wide note below the table. | 2 callers / 0 callees |
| `0x601019a6` |   12 | Diagnostics / JSON | **`rpc_arg_is_valid`** — Check if RPC argument descriptor is non-null and valid. | 1 caller / 0 callees |
| `0x601019b2` |   40 | Diagnostics / JSON | ~~**`rpc_arg_lookup_by_id`** — Look up RPC argument by ID in argument array (`0x60101984`).~~ ⚠️ **mischaracterized, corrected QA session 163** — no ID/array-lookup logic anywhere; it bounds-checks a length value and either stores it as the buffer's target length, or (on failure) falls back to writing the JSON `"null"` literal via `0x60101984` above. See the cluster-wide note below the table. | 1 caller / 1 callee |
| `0x601019da` |   56 | Diagnostics / JSON | ~~**`rpc_arg_serialize_uint32`** — Serialize 32-bit integer into RPC argument payload (13 callers).~~ ⚠️ **mischaracterized, corrected QA session 163** — this exact address is already independently established, and reused directly by the already-established `json_format_object_close` (`0x600d4566`) specifically to append the closing `}` delimiter — a generic bounds-checked "advance write position" primitive, not RPC-uint32-specific. See the cluster-wide note below the table. | 13 callers / 0 callees |
| `0x60101a12` |   66 | Diagnostics / JSON | **`rpc_arg_copy_bytes`** — Copy binary bytes into buffer with bounds checking (`0x6013d3a0`). | 1 caller / 1 callee |
| `0x60101a54` |   16 | Diagnostics / JSON | **`rpc_arg_init_header`** — Reset buffer to the JSON `"null"` literal and clear status flags (`0x60101984`, 5 callers). | 5 callers / 1 callee |
| `0x60101a64` |   28 | Diagnostics / JSON | ~~**`rpc_arg_get_payload_ptr`** — Return pointer to RPC argument payload data (12 callers).~~ ⚠️ **mischaracterized, corrected QA session 163** — its body writes the literal bytes `{`/`}` (`0x7b`/`0x7d`) and sets length 2 — a JSON empty-object initializer, matching the already-established `json_builder_init_object_256`/`_512`/`_200` family exactly (same `{`/`}` shape, different buffer size). No pointer is returned. See the cluster-wide note below the table. | 12 callers / 0 callees |
| `0x60101a80` |    8 | Diagnostics / JSON | **`rpc_arg_get_uint32`** — Thin one-line wrapper forwarding to `0x60101a64` above (13 callers). | 13 callers / 0 callees |
| `0x60101a88` |   28 | Diagnostics / JSON | ~~**`rpc_arg_get_uint16`** — Read 16-bit integer from RPC argument payload.~~ ⚠️ **mischaracterized, corrected QA session 163** — writes the literal bytes `[`/`]` (`0x5b`/`0x5d`) and sets length 2 — the JSON empty-array counterpart to `0x60101a64`'s empty-object initializer above, not an integer read. See the cluster-wide note below the table. | 1 caller / 0 callees |
| `0x60101aa4` |   36 | Diagnostics / JSON | **`rpc_arg_serialize_bytes`** — Append a raw byte buffer then a closing `}` delimiter (`0x600cc3cc`, `0x60101a12`, 8 callers). | 8 callers / 3 callees |
| `0x60101ac8` |   68 | Diagnostics / JSON | ~~**`rpc_arg_serialize_string`** — Serialize null-terminated string into RPC argument payload (4 callers).~~ ⚠️ **mischaracterized, corrected QA session 163** — its body is gated on the buffer *starting* with `[` and appends `, ` (comma-space) between elements, or advances past the initial `[` for the first element — the real JSON array-element-separator logic, not string serialization into an RPC payload. See the cluster-wide note below the table. | 4 callers / 0 callees |
| `0x60101b0c` |   64 | Diagnostics / JSON | ~~**`rpc_arg_deserialize_string`** — Deserialize string parameter from RPC request payload (13 callers).~~ ⚠️ **mischaracterized, corrected QA session 163** — its control flow (check sign, negate both halves of a value on the negative branch, call the established value-formatter `0x60050c18` on the magnitude, then conditionally prepend `-`) is a signed-integer-to-decimal-string *serializer* with sign handling — an output/formatting operation, the opposite of "deserialize... from RPC request payload." See the cluster-wide note below the table. | 13 callers / 0 callees |
...
**⚠️ QA session 163 note:** Backbone: all 37 addresses/sizes match the census exactly, fully contiguous `0x6010162e`–`0x60101c22`. Independently re-derived `bruce-decompile-status.md`'s totals via a fresh header-parsed join (3,805 functions / 548,162 bytes, 0 mismatches, 0 duplicates, 0 non-census addresses) — matches the wave's claimed 1,524-byte/76.67% exactly.

**Strong retroactive confirmation of session 161's correction**: this wave finally decompiles `0x60101ba2` and `0x60101b76` — the two functions cited by *dozens* of already-established rows throughout this doc as the "advance offset"/"append string" pair used alongside `0x60050c18` (the diagnostic-buffer value formatter session 161 corrected `kvs_calc_and_store_crc`'s citation of, away from "CRC"). Read in full: `log_formatter_advance_offset` (`0x60101ba2`, 56 real callers) is precisely a bounds-checked write-offset advancer with overflow/status handling, and `log_formatter_append_raw_string` (`0x60101b76`, 82 real callers) is precisely a bounded string-copy appender — exactly matching what session 161's correction required them to be, now confirmed directly rather than by citation alone.

**A significant correction to this wave's own "RPC Argument Serialization & Deserialization" section (11 of its 14 rows)**: reading each function's actual body shows this cluster is not RPC-specific at all — it is the *same* generic JSON/diagnostic-text-buffer-serialization engine this doc has documented extensively since sessions 61–62 (`json_format_*`/`json_builder_init_object_*`), continuing directly from this wave's own `log_formatter_*` section just above it in the same commit. The clearest, most concrete evidence: (1) `0x60101984` (claimed "validate RPC argument descriptor header") writes the literal ASCII bytes `"null"` — the JSON null-value literal, not a header check. (2) `0x60101a64`/`0x60101a88` (claimed RPC payload-pointer/uint16 getters) write literal `{`/`}` and `[`/`]` respectively — JSON empty-object/empty-array initializers, structurally identical to the already-established `json_builder_init_object_256`/`_512`/`_200` (same delimiter bytes, different buffer size). (3) `0x601019da` (claimed "serialize 32-bit integer into RPC argument payload") is the *exact* address the already-established `json_format_object_close` (`0x600d4566`) calls to append its closing `}` — a generic advance-position primitive, not integer-specific, confirmed by direct reuse rather than inference. (4) `0x60101ac8` (claimed "serialize string into RPC argument payload") is gated on the buffer *starting* with `[` and writes `, ` between entries — real JSON array-element-separator logic. (5) `0x60101b0c` (claimed "deserialize string parameter from RPC request payload") is unambiguously a signed-integer-to-decimal-string formatter (sign check, magnitude negation, calls the established `0x60050c18`, conditionally prepends `-`) — an *output* operation, the opposite direction from "deserialize... from a request." Corrected all 8 directly-contradicted rows in place (struck through the RPC framing, described the real JSON-primitive behavior); `0x601019a6`/`0x60101a12`/`0x60101a54`/`0x60101aa4` were left with lighter, hedged corrections since their underlying mechanics (bounds-check-and-set-flag, bounds-checked memcpy) are accurately described even though the "RPC argument" framing is unconfirmed. Real identity for the whole cluster: additional primitives in the same generic JSON/diagnostic-serialization engine, not a distinct RPC-argument subsystem — left unconfirmed as to whether *any* RPC-specific caller actually exists, rather than guessed.

**Other citations spot-checked and confirmed clean**: the RTOS queue/event/task primitives (`rtos_queue_init_lazy`, `rtos_queue_create_custom`, `rtos_event_wait`, etc.) all compose already-established `queue.c`/`event_groups.c`/`tasks.c` primitives (`0x601007bc`, `queue__600c9de4`, `tasks__600ca27c`, `tasks__600ca5cc`) consistently; the KVS sector-header/payload functions (`0x60101848`/`0x601018de`) are called by the already-established `key_value_store__*` functions and match a coherent chunked-copy shape mirroring each other (write vs. read) precisely.
...
| `0x6010209a` |   14 | C Runtime / Panic | **`system_fatal_abort`** — System fatal abort handler invoking `exit` (`0x60051824`) and panic handler (32 callers). ⚠️ **decompiled-content caution, QA session 164** — the row's own claim (call the panic handler, then `exit(1)`) is well-supported and matches this function's own genuine 14-byte size and its 2 real callees exactly, but the *committed `.c` file's body* renders a full binary-search algorithm reading `extraout_r2`/`in_r3`/`extraout_r3` — the well-established "tail-call bleed" artifact signature (sessions 30/49/52/53/56/61/90) — meaning that visible pseudocode belongs to some other, uncataloged function reached via a tail call, not to this one. **GHIDRA-TODO: mark the tail-call edge out of `0x6010209a` explicit via `FixTailCalls.java`** (the proven fix from session 53) and re-decompile. | 32 callers / 2 callees |
...
**⚠️ QA session 164 note:** Backbone: all 28 addresses/sizes match the census exactly, fully contiguous `0x60101c22`–`0x60102192`. Independently re-derived `bruce-decompile-status.md`'s totals via a fresh header-parsed join (3,832 functions / 549,520 bytes, 0 mismatches, 0 duplicates, 0 non-census addresses) — matches the wave's claimed 1,392-byte/76.86% exactly.

**A flagship-tier retroactive confirmation, ~92 sessions after the original finding**: session 72's QA independently identified a genuine `std::map`/`_Rb_tree` implementation purely from *call-site* evidence — most conclusively, `0x60101d4e`'s argument shape matching real libstdc++ `_Rb_tree_insert_and_rebalance(bool, _Rb_tree_node_base*, _Rb_tree_node_base*, _Rb_tree_node_base&)` argument-for-argument, without ever seeing this function's own body. This wave finally decompiles it, and the *body* now confirms the identification completely: new-node initialization (zero children/parent, color set to red), the standard insert-fixup case analysis, and calls to this wave's own confirmed `rbtree_rotate_left`/`rbtree_rotate_right` (`0x60101c78`/`0x60101ca2`) — an exact match to the textbook `_Rb_tree_insert_and_rebalance` algorithm. `rbtree_delete_rebalance` (`0x60101e12`, 436 bytes) likewise matches the real `_Rb_tree_rebalance_for_erase` case structure (successor-splice-then-fixup with sibling/nephew color checks). `rbtree_find_min_max` (`0x60101ccc`) is also independently confirmed by session 72's own citation of its veneer, `thunk_FUN_60101ccc`, as a "decrement-shaped helper" — matching exactly.

**A second, independent confirmation reaching back to session 48**: `minimal_vsnprintf`'s (`0x6010201a`) buffer-overflow path calls `FUN_600cc9e4` — session 48 already established that exact address embeds the real GCC/libstdc++ diagnostic string `"not enough space for format expansion..."`. A minimal vsnprintf implementation calling that specific diagnostic precisely when its output buffer is full is exactly the context that finding predicted, now confirmed from the calling side 116 sessions later.

**One well-evidenced decompiler-artifact finding**: `system_fatal_abort`'s (`0x6010209a`) committed `.c` file renders a full binary-search algorithm using `extraout_r2`/`in_r3`/`extraout_r3` — the well-cataloged "tail-call bleed" signature (sessions 30/49/52/53/56/61/90) — meaning the visible pseudocode belongs to a different, uncataloged function reached via an unmarked tail-call edge, not to this genuine 14-byte function. The row's own prose claim doesn't repeat the fabricated content and remains well-supported by the function's real size and its 2 correctly-listed callees; flagged with a `GHIDRA-TODO` recommending the session-53 `FixTailCalls.java` fix rather than corrected outright.
...
**⚠️ QA session 165 note:** Backbone: all 14 addresses/sizes match the census exactly, fully contiguous `0x60102192`–`0x60102746`, zero overlaps. Independently re-derived `bruce-decompile-status.md`'s totals via a fresh header-parsed join (3,844 functions / 550,896 bytes, 0 mismatches, 0 non-census addresses, 0 duplicates) — matches the wave's claimed 77.05% exactly.

**Two confirmed fixes, verified directly rather than trusted from the commit message**: (1) session 164's `system_fatal_abort` (`0x6010209a`) `GHIDRA-TODO` is genuinely resolved — the file now cleanly decompiles to `FUN_600cdc90(); exit__60051824(1); return;` (`raise()` then `_exit(1)`), zero `extraout_r2`/`in_r3` artifacts, an exact match for the real newlib/POSIX `abort()` implementation. (2) `0x601023fa` — labeled `posix_signal_dispatch` in this wave's table — is the same address as `_raise_r`, established since session 50 and chased through the sessions 50–53 tail-call-bleed saga (`analysis/bruce-misc-functions.md` line ~1695); its decompiled `.c` file is now finally clean too (no fabricated `FUN_600ce1e8` call, no `unaff_rN` tokens), matching that row's long-standing prose exactly. This fresh, full-body view adds new exact-precision confirmation of four real POSIX/newlib constants not previously checked numerically: `NSIG`=32 (`sig < 0x20`), `SIG_IGN`=1, `SIG_ERR`=-1 (`0xffffffff`), and `EINVAL`=`0x16`=22 decimal (set on both the out-of-range and `SIG_ERR` paths). `0x6010244a` (`thunk_FUN_600ce1d8` in this wave) likewise matches the already-corrected `_getpid_r`-thunk row exactly (a plain veneer into `getpid`/`0x600ce1d8`). Recommend the next wave cite these established names (`_raise_r`, `_getpid_r`-thunk) rather than the fresh generic labels, now that both are independently re-confirmed twice over.

**One confirmed misidentification, ~116 sessions old**: `0x600cdbbc`, long established as `__getreent`, is actually a ctype-table-pointer accessor — corrected in place above (session 51-era row) with full evidence from this wave's own `strcasecmp_custom` (`0x6010244e`), cross-validated against session 164's `ctype_is_xdigit_or_blank` using the identical return-value convention. The real BSD/newlib ctype bit semantics (`_U`=0x01, `_L`=0x02) and the `+0x20` upper→lower case-fold literal both match exactly, while the callee's own body (an *extra* dereference to `*(reent+0xec)`, not the reent pointer itself) directly contradicts what `__getreent()` should return.
...
**⚠️ QA session 166 note — MAJOR FINDING: 7 of 9 functions sit in a region Ghidra itself flags as corrupted disassembly, matching a class already documented since session 6.** Backbone (address/size) checks out clean: all 9 addresses/sizes match the census exactly, summing to the claimed 1,780 bytes, no overlaps (the large gaps between entries — up to 4,268 bytes — are simply undecompiled census territory, not a boundary problem). Independently re-derived `bruce-decompile-status.md`'s totals via a fresh header-parsed join (3,853 functions / 552,676 bytes, 0 mismatches, 0 non-census) — matches the wave's claimed 77.30% exactly.

But this wave drew its addresses from inside `bruce-decompile-status.md`'s own §3b row 2 (`0x600ecb50`–`0x6013d4e4`, a 330,132-byte unattributed span) — the *exact* range whose own table entry cites `FUN_601054dc` (2,546B @ `0x601054dc`) as a landmark, and which `CLAUDE.md`'s "Conventions & gotchas" section has flagged since **session 6** as containing "genuine disassembly corruption — likely an unrecognized Thumb-2 `TBB`/`TBH` jump table — not a real function boundary. Needs a manual jump-table fixup in Ghidra before it's usable, not another blind decompile attempt." This wave appears to have hit exactly that hazard, independently confirmed via direct Thumb-2 disassembly of the raw flash image (`reference_firmware/bruce_pvt_a_prod_signed.bin`), cross-checked against each file's own decompiler warning comments:

- **`0x60131a30` (`dsp_driver_init_channel`)**: raw disassembly matches Ghidra's pseudocode cleanly for the first ~12 instructions (a function-pointer-table check/call, then a byte-flag test), but the flag-test's zero-branch target (`0x60131a72`) — reached exactly when Ghidra's own `if` condition is false — disassembles to pure garbage: a chaotic, non-flowing sequence of unrelated `cmp`/`strh`/`ldrb`/`ldm` instructions including an `mrc p8, #7, r3, c7, c9, #7` (coprocessor-8 access — **not a valid instruction on Cortex-M7**, which has no classic MRC/MCR coprocessor bus). This is data (very plausibly a coefficient/LUT table or a jump table Ghidra didn't recognize), not code — matching the file's own `WARNING: Control flow encountered bad instruction data` / `Bad instruction - Truncating control flow here` markers exactly.
- **`0x60131e14`, `0x601323f6`**: both marked `Bad instruction data` / `Truncating control flow`; both also reference **Cortex-M exception-vector-table symbol names** (`_UsageFault`, `_MasterStackPointer`, `_Reset`, `Reserved1`/`Reserved4`) as if they were ordinary data globals — a strong tell that computed addresses are resolving into low memory near the real vector table (`0x0`) rather than anything DSP-related, consistent with corrupted control flow rather than real code.
- **`0x60132bb4`** (the wave's largest, 548B): raw disassembly confirms the *opening* ~16 instructions are genuinely clean and match Ghidra's pseudocode precisely (a `bfi`/`bfi`/`bfi` bit-packing sequence building a 16-bit value from three fields, passed to `FUN_600b4ec2(0x63, ...)` — this part is very likely real). But deeper in, Ghidra's own comments show `Could not recover jumptable at 0x60133174. Too many branches` and again at `0x60132f40`, plus 12+ `Removing unreachable block` warnings and an embedded `software_bkpt(0xf7)` — textbook symptoms of an unrecognized `TBB`/`TBH` switch dispatch.
- **`0x60132e5c`**: flagged `Instruction at (ram,0x60133084) overlaps instruction at (ram,0x60133082)` — two decoded instructions claiming overlapping byte ranges is only possible when the disassembler has lost correct instruction alignment, plus 12 more `Removing unreachable block` warnings.
- **`0x60134088`** (`bb_coproc_packet_dispatch`) and its sole callee **`0x60134126`** (`bb_coproc_queue_slot`): both carry `extraout_r1`/`extraout_r3`/`unaff_r9`/`unaff_r6`/`unaff_r7` register artifacts (the standard HANDOFF Rule 8 tell), and `0x60134088`'s body embeds a `software_interrupt(0x78)` mid-arithmetic with no plausible purpose — the claimed "`mcrr2 p2`" coprocessor acceleration trigger cited in the commit message is architecturally implausible on its face: the BCM43458 radio is an **external, UART/SPI-attached chip** (per this project's own long-established HCI H4 UART transport findings), not an on-die ARM coprocessor reachable via `MCRR`/`MCRR2` register-transfer instructions, which Cortex-M7 does not implement for general peripheral access in the first place.

**Two functions are independently confirmed clean and reliable**, verified via direct disassembly matching Ghidra's pseudocode instruction-for-instruction with zero artifacts: **`0x60132de4` (`dsp_stream_param_negotiate`)** — a genuinely coherent min-of-two/struct-copy/dual-callee-compare function, no corruption; and **`0x60134018` (`bte_calc_packet_slots`)** — not only clean but a precise, independently-checkable real Bluetooth Core Spec match: `0x271` = 625 decimal = the real Bluetooth baseband time-slot duration in microseconds, and `0x369` = 873 decimal, both used exactly as the commit message's own gloss claims. This row is fully confirmed.

**Recommendation, not applied this session (out of QA scope — needs Ghidra GUI access)**: `GHIDRA-TODO`, matching the session-6 `FUN_601054dc` precedent exactly — the 7 flagged addresses (`0x60131a30`, `0x60131e14`, `0x601323f6`, `0x60132bb4`, `0x60132e5c`, `0x60134088`, `0x60134126`) need manual jump-table/data-boundary investigation in the Ghidra GUI (very likely one or more unrecognized `TBB`/`TBH` tables embedded as inline data within or adjacent to these claimed function bodies) before their full decompiled content — and therefore the "DSP audio pipeline"/"baseband coprocessor" subsystem framing built on top of it — can be trusted beyond the clean opening fragments identified above. Not struck through (the addresses/sizes and the *opening* instructions of several are real and may anchor genuine functions once correctly bounded), but flagged as unreliable pending that fixup, consistent with `FUN_601054dc` having sat unresolved since session 6 as "not another blind decompile attempt."
