...
| Address | Bytes | Subsystem | Functional Role & Evidence | Call graph |
|---|---:|---|---|---|
| `0x600ee89c` |   86 | BTA / GATTC | **`bta_gattc_search_service_dispatcher`** — Service discovery dispatcher: queries cache control block via `0x6009506c` (`bta_gattc_find_srvc_cache`); if discovery active (`state == 2`) delegates to `0x600ee8f2`, else calls `0x6009496c` (`bta_gattc_search_service`). | 1 caller / 3 callees |
