...
| `0x600edaf6` |    4 | Crypto / LHASH | **`crypto_lh_num_buckets`** — BoringSSL internal `lh_num_buckets`: returns `lh->num_buckets` (`return *(lh + 4)`). | 0 callers / 0 callees |
...
| `0x600edafe` |   38 | Crypto / LHASH | **`crypto_lh_get_item_at_index`** — Retrieves item from internal stack/list at index `param_2`, bounds-checking against `0x600e0ab2` count and calling `0x600e0ab8`. | 1 caller / 2 callees |
