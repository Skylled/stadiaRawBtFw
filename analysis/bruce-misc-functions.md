...
| Address | Bytes | Subsystem | Functional Role & Evidence | Call graph |
|---|---:|---|---|---|
| `0x600efa30` |   82 | BTA / GATTC | **`bta_gattc_find_bg_conn_slot_by_app_id`** — Background connection lookup: searches 10-slot table (32-byte stride at `param_1 + 4`), matching `app_id` at `+0x20`, returning slot index (0..9) or 0xff if not found. | 1 caller / 0 callees |
...
| `0x600efd42` |   32 | BTA / DM Search | **`bta_dm_disable_search_if_device_up`** — DM Search state clear: calls `0x600ee278` (`btm_is_device_up_locked`); if active, clears search flag 0 via `0x60095f38` (`bta_dm_search_set_state_flag(0)`). | 0 callers / 2 callees |
