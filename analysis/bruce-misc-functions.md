...
| `0x600dbf20` |  28 | Synapse / Cleanup | **`synapse_audio_processor_destructor_free`** — 1140-byte Synapse audio processor destructor: cleans processor sub-structures via `FUN_600dc30e` and frees 1140-byte object (`0x474`) via `thunk_EXT_FUN_0000b52e(iVar1, 0x474)`. Called by `0x6007b164` and `synapse_audio_processor__6007b1b8`. | 2 callers / 2 callees |
...
| `0x600dc216` |  50 | Container / Vector | **`vector_geometric_growth_capacity`** — `std::vector` geometric capacity calculator: calculates current vector size `uVar2 = (end - begin) >> 2`, checks maximum capacity `0x3fffffff`, computes new geometric growth capacity `max(size, requested_add)`, handles overflow (calls `FUN_60101fd8`), returns new capacity. Called by 4 callers across audio buffer vector resizing (`0x6007b9d4`, `0x6007ba54`, `0x6007c3e0`, `0x6007c4c8`). | 4 callers / 1 callee |
