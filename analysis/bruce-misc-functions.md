...
| `0x600efe24` |   26 | BTA / DM PM | **`bta_dm_pm_set_conn_params`** — PM connection parameters dispatcher: delegates to `0x60096e38` (`bta_dm_pm_set_params`). | 1 caller / 1 callee |
...
| `0x600f00cc` |  256 | BTA / Vendor | **`bta_dm_pm_send_vs_mode_cmd`** — Vendor-specific PM mode dispatcher: finds PM record via `0x600bb6dc` (`btm_pm_find_rec`), sends vendor HCI opcode `0xfc57` (mode 1, 3B) or `0xfd1a` (mode 2, 4B) via `0x600a1394`, and triggers mode update via `0x600b8f0c`. | 1 caller / 3 callees |
...
| `0x600f0252` |  232 | BTA / DM PM | **`bta_dm_pm_set_sniff_subrating`** — Sniff subrating configuration: checks supported features via `0x600a1354` (`BTM_ReadLocalSupportedFeatures`), masks unsupported flags, looks up link block via `0x60098ec8`, and invokes `0x600b2f80` (`BTM_SetSniffSubrating`). | 5 callers / 3 callees |
