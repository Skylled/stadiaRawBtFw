...
| Address | Bytes | Subsystem | Functional Role & Evidence | Call graph |
|---|---:|---|---|---|
| `0x601023fa` |  80 | C Runtime / Signals | **`_raise_r`** — Reentrant signal raiser: checks `sig < 0x20`, looks up signal table at `param_1 + 0x44`, dispatches registered handler `handler(sig)`, or default handles via `_kill_r(_getpid_r(param_1), sig)` (`FUN_600cdca0`). This row's *prose* is disassembly-confirmed accurate (verified independently, QA session 50 — the real final branch is a tail-call `b.w 0x600cdca0`, matching exactly). ⚠️ *(QA session 50)* but **the committed `analysis/decomp/FUN_601023fa.c` is not fixed** despite the Session 49 `GHIDRA-TODO` boundary now existing: it still shows `unaff_r4`/`unaff_r5`/`unaff_lr` reads and a call to `FUN_600ce1e8` with 8 arguments (including those 3 uninitialized ones) — but `FUN_600ce1e8` takes **zero** real parameters (see its own row below) and is not even the real target; the true tail-call target is `0x600cdca0` (`_kill_r`). Defining the boundary was necessary but not sufficient — the file needs re-decompiling. **GHIDRA-TODO: re-run Decompile.java on `0x601023fa` (and `0x600cdc90`, see below) after locking `FUN_600ce1e8`'s signature to zero parameters; expect both to redecompile without `unaff_` artifacts.** | 0 callers / 1 callee |
