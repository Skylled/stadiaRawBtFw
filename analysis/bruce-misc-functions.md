...
| `0x600cf0ca` | 232 | Hardware / I2C | **`i2c_transfer_submit`** — I2C master transaction submitter: verifies bus state via `FUN_600cefde`, formats 16-bit packet control words (`*(param_2 + 8) = ... | 0x400`), configures mode flags, and arms peripheral registers. ⚠️ *(QA session 53, weaker finding)* the decomp body carries an `extraout_r2` artifact: `FUN_600cefde` is called with 0 visible args and its known-32-bit return (`undefined4`) is treated as a 64-bit split value (`(ulonglong)uVar12`), with the "high half" used as a pointer (`*extraout_r2` etc.) — the same call-site parameter/return-width mismatch class as the raise/_raise_r bug, just smaller in scope. The overall identity and general shape of this row are still plausible, but don't cite the specific `iVar8`/`extraout_r2`-derived field offsets as verified — they're likely decompiler noise, not real fields. | 0 callers / 1 callee |
