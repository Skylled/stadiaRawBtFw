...
- **XBARA1 Signal Trigger Routing (`xbara__60060170`, Wave 3)** — `xbara__60060170` @ 0x60060170 (`src: xbara.h`, 74B) implements `XBARA_SetSignalsConnection` targeting the Crossbar Switch (XBARA1 base `0x403BC000`). It programs the 16-bit `XBARA_SEL0..SEL29` registers at offsets `0x00..0x3A` to multiplex internal timer/PWM/GPIO trigger pulses into ADC External Trigger Control (ADC_ETC) inputs, enabling hardware-synchronized analog joystick sampling.
