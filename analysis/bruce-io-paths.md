...
- **`usb_host_hid__60064dac`** (494B) — device attach/detach handler (a 3-state switch: attach=2, detach=1, "already attached, ignore"=3). On attach, reads the interface descriptor's VID/PID (`FUN_600d33b2`) and walks two fixed built-in tables of known VID/PID pairs to pick a per-device profile: one table (ending in a `uVar4==0x28e`/`0x719` check) recognizes vendor `0x054c` (Sony)'s **PID `0x028e`** (a DualShock 3) and **PID `0x0719`** (a Sixaxis/PS3-family wireless dongle), each mapped to a distinct "**PS3 init packet**" magic-byte blob (`DAT_60064fa4`/`DAT_60064fa8`/`DAT_60064fac`) and expected-response-length constant (`3`/`0xc`/`5`) — the well-known third-party-PS3-controller "magic 0xF4 feature-report" handshake real PS3 controllers need before they'll start sending input reports over USB. A second table matches by HID-interface-subtype (boot-protocol byte checks) for **Nintendo Switch Pro** (vendor `0x057e` PID `0x2009`) and a Sony vendor `0x28de`-prefixed family (PIDs `0x1102`/`0x1142`) — routing those to `switch_pro_controller.cc`'s driver instead (this function only classifies the device and stashes the matched init-packet pointer; it doesn't itself parse Switch Pro reports).
...
**Net**: `bruce` statically links a small **generic USB-host HID class driver with hardcoded quirk tables for three specific third-party gamepad families** (Sony DualShock 3 / Sixaxis PS3 dongle, Nintendo Switch Pro, and a second Sony `0x28de`-vendor family) sitting alongside the dedicated `switch_pro_controller.cc` report parser — i.e. the Stadia controller's firmware image was built to (at least in principle) let something be plugged into a USB **host** port and read as a gamepad, with the PS3-family devices specifically requiring (and receiving) the standard "magic init packet" real third-party PS3 controllers need. Same standing caveat as `bruce-audio-subsystem.md`'s USB-host-audio findings: **no caller into this driver's entry points was found**, and the physical Stadia BT controller has no exposed USB host port, so this is very likely more shared-platform code from the same non-controller product family, not active on this SKU.
