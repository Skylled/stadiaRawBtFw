...
- **Confirmed i.MX RT106x MMIO bases present in the image**: GPIO1–5 = 0x401B8000 / 0x401BC000 / 0x401C0000 / 0x401C4000 / 0x400C0000; ADC1/ADC2 = 0x400C4000 / 0x400C8000; XBARA1 = 0x403B0000 / 0x403BC000; IOMUXC pad registers 0x401F80CC / 0x401F82BC — all referenced from two parallel 5-entry "GPIO bank" tables at **0x60108a70–0x60108a80** and **0x60109210–0x60109220**.
- **XBARA1 Signal Trigger Routing (`xbara__60060170`, Wave 3)** — `xbara__60060170` @ 0x60060170 (`src: xbara.h`, 74B) implements `XBARA_SetSignalsConnection` targeting the Crossbar Switch (XBARA1 base `0x403B0000`). It programs the 16-bit `XBARA_SEL0..SEL29` registers at offsets `0x00..0x3A` to multiplex internal timer/PWM/GPIO trigger pulses into ADC External Trigger Control (ADC_ETC) inputs, enabling hardware-synchronized analog joystick sampling.
- **Board Diagnostic Health Supervisor (`timer__60074658`, Wave 3)** — `timer__60074658` @ 0x60074658 (`src: timer.h`/`board.cc`, 538B) is the periodic board supervision callback. It probes TS3A227E (accessory detect), WM8904 (audio codec), BQ2742X (battery fuel gauge), TUSB320 (USB-C port controller), USB device, and haptic actuators, spawning a 1000ms periodic health timer and triggering a system bug report via `trigger_bug_report__6005d714` upon detecting component communication faults.
